...
- Assurance tier, declared per unit at planning: `kernel` (judgment-bearing code/spec) = full adversarial battery; `data` (consumer re-validates at use) = one structural validator + live spot-check, defects fix forward; `docs` = consistency pass. Deciding question: where does a defect get re-checked for free, and what does it cost there? Re-proving a downstream check upstream = waste, not rigor.
- Milestone = MVP spine — the shortest unit path to a consumable artifact. Off-spine improvements are born as `.agent/polish.md` entries with acceptance checks written at deferral time, never as units; `/session-polish` consumes them on spare capacity.
...
- `/session-polish` = the polish register's sole consumer: stateless, any-order, zero milestone coupling; guards — never raise a tier, add units, or edit scope sources.
