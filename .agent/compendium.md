...
when its row is promoted into `.agent/queue.md` (procedure:
`docs/REFERENCE.md` § Operating).
...
- `status` = `unqueued | provisional(<why>) | queued | in-progress |
  ...
  worked to completion per `docs/REFERENCE.md` § Operating. Ruled-out rows
...
- `notes` carries `id=<id>` once fetched (id rules: `docs/REFERENCE.md`
...
promoted: row status → `queued`, one queue entry appended per
`docs/REFERENCE.md` § Operating; on fetch the row gains `id=<id>` and follows the
