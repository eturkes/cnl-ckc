...
- M5.6 dist (shell tier, pinned dep; contract `.agent/contracts/m5u6.md` R71–R72 + R85; red suite on `wt/test-m5u6` 7fcf4890 (test-m5u6-1: `rust/ckc/tests/dist_cases.rs` 62/62 red on main, 62/62 green via a legacy argv adapter; `.scratch/m5u6/dist_diff.py` self-check 1047/1047); spec `release.rs` digest_lines/tagmanifest_lines on main; kernel seed on `wt/prod-m5u6` (2 red stubs) → prod-m5u6-1). Accept: 62 red cases re-expressed; `sha256sum -c` + double-build determinism; FC4 re-baseline.
