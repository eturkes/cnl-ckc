...
- M5.4 pipeline (`ckc compile|queries|align|review-manifest|release-manifest|ledger-validate`) + fork shrink (contract `.agent/contracts/m5u4.md` R65–R66; spec items on main: `align.rs` § resolver, `release.rs`; diff-blind harnesses: `.scratch/m5u4/{align_diff.py,align_probes/run.py,parity.py,expected/}` (test-m5u4-1); kernel seed on `wt/prod-m5u4` (2 red stubs: `align_resolve`, `release_manifest`) → prod-m5u4-1 (kernel + shell); fork shrink P6 = MAIN). Accept: corpus + manifest byte-identical.
