...
- M5.2b K3 trace + trace-check (IN PROGRESS, `.agent/contracts/m5u2b.md`, R32–R39 + R54–R56 + R60; spec landed; `just kani` landed, P3 partial; kernel on `wt/prod-m5u2b` (prod-m5u2b-2: runtime bindings implemented, proofs closing), theorem on `wt/prod-k3sound` (prod-k3sound-1), grading harness = test-m5u2b-2: `.scratch/m5u2/diff/k2_corpus_diff.py --lanes trace`, `.scratch/m5u2/suite/activate_k3.py --verify`, `.scratch/m5u2b/queries_replay.py`). Accept: lanes D/E byte-identical; tests/queries replay; `k3_sound` discharged; P1–P7.
- M6 emission certification (IN PROGRESS, `.agent/contracts/m6.md`, R40–R43 + R50–R53; spec `emit.rs` + driver `drs_dump.pl` landed; kernel on `wt/prod-m6` (prod-m6-2: 1238/2, projection in progress)). Accept: `ckc certify` green over every committed document + query in CI; `ace_to_pl.pl` leaves the human-read trust story.
- M5.3 validators (K4; IN PROGRESS, `.agent/contracts/m5u3.md`, R44–R49 + R57–R64; spec `check.rs` landed + constructive/byte-exact amendments; exec surface + kernel on `wt/prod-m5u3` (prod-m5u3-1)). Accept: `ckc check` section parity with `goal.py check`; red/adjudication replay green; FC2 re-pins ruled.
