...
- M5.3 validators (K4; K3 WIRING IN PROGRESS = prod-m5u3-2 on `wt/prod-m5u3`, `.agent/contracts/m5u3.md` R44–R49 + R57–R64 + R68 + R74 + R77–R81): kernel complete on `wt/prod-m5u3` 02f8677f (prod-m5u3-1: 1291/0, all K4 bindings; allowlist 44 sites per R78); shell complete on `wt/prod-m5u3s` f7d1aac7 (prod-m5u3s-1: `ckc check` dispatcher + every in-scope section; K3 seams named rc2 boundaries) being merged into `wt/prod-m5u3`; remaining: R80 `tests/queries/r79-nonv1.tsv`, R81 probe, K3 wiring after M5.2b lands, `just rust` in the worktree, lead parity (`.scratch/m5u3/parity.py`, `mutants.py` 46, 155 battery pins, 4717 s legacy baseline), P6 wall. Accept: `ckc check` section parity with `goal.py check`; red/adjudication replay green; FC2 re-pins ruled (R77/R79).
- M5.4 pipeline (`ckc compile|queries|align|review-manifest|release-manifest|ledger-validate`) + fork shrink (contract `.agent/contracts/m5u4.md` R65–R66; spec items landed: `align.rs` § resolver, `release.rs`; diff-blind harnesses ready: `.scratch/m5u4/{align_diff.py,align_probes/run.py,parity.py,expected/}` (test-m5u4-1); prod not yet dispatched — branch from `wt/prod-m5u3` after M5.3 + M6 land on main). Accept: corpus + manifest byte-identical.
- M5.5 UI (K5; contract `.agent/contracts/m5u5.md` R69–R70 + R73 + R76 + R84; spec `rust/ckc-spec/src/ui.rs` on main (R84: read in full + accepted, 2859 lines, workspace verifies); seed 8d9dbc83 on `wt/prod-m5u5` (14 red stubs) → prod-m5u5-1 (kernel) + test-m5u5-1 (`wt/test-m5u5`) running; shell teammate follows the kernel's page-render checkpoint; R87–R89 ruled). Accept: pages byte-identical vs `ui.py render`; tests/ui 84 red + 15 green; POST/CAS/committed-corpus law verbatim; fidelity theorems.
- M5.6 dist (shell tier, pinned dep; contract `.agent/contracts/m5u6.md` R71–R72 + R85; red suite done on `wt/test-m5u6` 7fcf4890 (test-m5u6-1: `rust/ckc/tests/dist_cases.rs` 62/62 red on main, 62/62 green via a legacy argv adapter; `.scratch/m5u6/dist_diff.py` self-check 1047/1047); prod after M5.4). Accept: 62 red cases re-expressed; `sha256sum -c` + double-build determinism; FC4 re-baseline.
