...
- M2.1 OPEN: vendor APE verbatim at pin `5f4d5354` under `vendor/APE/` + fork-pin scaffold mirroring e-- (PROVENANCE w/ LGPL-3.0-or-later modification-notice contract, MANIFEST.sha256, ordered `patches/`, fidelity procedure); update `docs/provenance.md`; vendor-time re-verification (M0 review decision): `make install` → runnable `ape.exe` on SWI 9.2.9, direct source-load parses the anchor, upstream `test_ape.pl` 3733/3733 w/ full Clex fetched at pin `20960a5c` (SHA-256 `2996fabf…`) into gitignored `.scratch/` (test-only, never committed); CI job: build + digest-pinned-Clex regression; CI must provision SWI == 9.2.9 (noble ships older, PPA stable now serves 10.x → apt-pin from PPA pool or cached source build at `e3b19512`; decide here).
- M2.2 OPEN: hand-Prolog adapter (`src/prolog/`): source-load vendored APE, call `ace_to_drs:acetext_to_drs/8` (Guess=off, Catch=off), bypass timed `get_ape_results`; fail-closed contract = any APE message/warning or failed parse ⇒ structured error, exit ≠ 0, zero DRS bytes; project-owned canonical serializer (quoted/canonical, deterministic referent naming, no time/path/banner); fresh-SWI-process stdin→stdout byte contract; golden fixtures (anchor `John waits.` → `drs([A],[-(predicate(A,wait,named('John')),/(1,2))]).`, determiner+relative-clause, if-then, negative cases); determinism gate = two fresh processes `cmp` byte-identical; `tests/adapter-harness.sh` + CI.
