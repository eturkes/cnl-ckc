...
- M3.1 OPEN — IR v1 spec + validator + CLI framing: `docs/ir.md` (term grammar, IDs/provenance, semantics + truth/unknown statement, safety, NAF representation + stratification rule, validator pass order + error classes, canonical serialization, deferral table); `src/prolog/ir_validate.pl` + `ir_tool.pl` (`validate` subcommand + shared framing: strict UTF-8 stdin, buffered stdout, canonical fixed-point check, error emission); `tests/ir-validate-harness.sh` (green/red: version/envelope/shape/identity/scope/safety/ordering/canonical-bytes/UTF-8/stream+exit classes) + CI wiring (lint list + swipl job). No APE dependency. Projected ≤130K.
- M3.2 OPEN (gate M3.1) — DRS→IR lowering: `drs_to_ir.pl` + `lower` subcommand; total mapping over the admitted profile (facts incl. copula normalization, one `=>` rule with positive multi-premise antecedent, one final `question/1`); event-referent erasure; provenance mapping; generate + commit both slice record fixtures via the front end (slice + slice-unknown variant without `John waits.` for the not-proved lane) with IR goldens; `tests/ir-lower-harness.sh` APE-free over committed records (red: wh-query, zero/multi question, be/4 variants, referent reuse, unknown constructors/fields/placements, non-canonical input). Projected ≤150K.
- M3.3 OPEN (gate M3.2) — compiler + kernel + explanation: `ir_to_prolog.pl`, `inference_kernel.pl`, `explanation.pl` + `compile`/`run` subcommands; program/result goldens for both slice records (proved + not-proved); replay gate, tamper rejection, private-module lifecycle, run-twice byte equality; `tests/ir-run-harness.sh` APE-free. Projected ≤185K.
