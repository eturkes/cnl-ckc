...
- M2.3 OPEN: user-lexicon mechanism: per-run `ulex:discard_ulex/0` + `ulex:read_ulex/1` (UTF-8 stream); OOV fail-closed fixtures (unknown noun w/o ulex ⇒ structured error; with `noun_sg(zorbomat,zorbomat,neutr).` ⇒ parse); override + isolation fixtures (ulex overrides Clex entry; no cross-run leakage); ulex file contract documented (entry forms, determinism duties, generation-from-registry note for M5).
