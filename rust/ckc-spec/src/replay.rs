...
// --- outputs ---
pub ghost struct Out {
    pub rc: int,
    pub out: Seq<u8>,
    pub err: Seq<u8>,
}

pub open spec fn atom(s: Seq<char>) -> Term {
    Term::Atom(ascii(s))
}
...
pub open spec fn check_load(detail: Term) -> Out {
    reject(2, "check_load"@, detail)
}

pub open spec fn proof_fail(detail: Term) -> Out {
    reject(1, "proof"@, detail)
}

pub open spec fn ok(out: Seq<u8>) -> Out {
    Out { rc: 0, out, err: Seq::empty() }
}
    ...
    Term::Comp(
        ...
        seq![
            Term::Comp(
                ...
                seq![
                    atom("ace_to_pl"@),
                    Term::Comp(ascii("/"@), seq![atom(pred), Term::Int(arity)]),
                ],
            ),
            arg,
        ],
        ...
        Term::Comp(
            ...
            seq![
                ...
                ctx(
                    "read_utf8_input"@,
                    3,
                    Term::Comp(ascii("byte_offset"@), seq![Term::Int(off as int)]),
                ),
            ],
        ),
    )
    ...
    decreases bytes.len() - i,
{
    if i >= bytes.len() {
        bytes.len()
    } else if bytes[i as int] == b {
        i
    } else {
        first_byte(bytes, b, i + 1)
    }
}
    ...
    decreases bytes.len(),
{
    if bytes.len() == 0 {
        ...
        if n >= bytes.len() {
            ...
            seq![bytes.take(n as int)] + lines_of(bytes.skip(n as int + 1))
        }
    }
}
...
// Bad = first invalid UTF-8 byte offset
pub open spec fn src_bytes(s: Src) -> Seq<u8> {
    match s {
        Src::Bytes(b) => b,
        _ => Seq::empty(),
    }
    ...
    decreases paths.len() - i,
{
    if i >= paths.len() {
        Option::None
    } else if srcs[i as int] is Missing {
        Option::Some(paths[i as int])
    } else {
        first_missing(paths, srcs, i + 1)
    }
}
    ...
    decreases rows.len() - i,
{
    if i >= rows.len() {
        acc
    } else if acc.contains(rows[i as int].pl) {
        unique_from(rows, i + 1, acc)
    } else {
        unique_from(rows, i + 1, acc.push(rows[i as int].pl))
    }
}

pub open spec fn unique_paths(rows: Seq<MRow>) -> Seq<Seq<u8>> {
    unique_from(rows, 0, Seq::empty())
}

pub open spec fn row_index(rows: Seq<MRow>, p: Seq<u8>, i: nat) -> nat
    decreases rows.len() - i,
{
    if i >= rows.len() || rows[i as int].pl == p {
        i
    } else {
        row_index(rows, p, i + 1)
    }
}

pub open spec fn src_of(rows: Seq<MRow>, pls: Seq<Src>, p: Seq<u8>) -> Src {
    pls[row_index(rows, p, 0) as int]
}
...
pub open spec fn first_bad_member(
    rows: Seq<MRow>,
    pls: Seq<Src>,
    ups: Seq<Seq<u8>>,
    i: nat,
    ...
    decreases ups.len() - i,
{
    if i >= ups.len() {
        ...
        if s is Bytes && accepts(src_bytes(s)) {
            first_bad_member(rows, pls, ups, i + 1)
        } else {
            Option::Some(ups[i as int])
        }
    }
}
    ...
    decreases ms.len(),
{
    if ms.len() == 0 {
        ...
        match ms[0] {
            V1File::Doc(d) => seq![d] + docs_of(ms.drop_first()),
            _ => docs_of(ms.drop_first()),
        }
    }
}
...
pub open spec fn db_of(docs: Seq<DocFile>) -> Seq<DocClause> {
    docs.map_values(|d: DocFile| doc_db(d)).flatten()
}
...
pub open spec fn wf_obs(obs: Seq<Ob>) -> bool {
    forall|i: int|
        0 <= i < obs.len() ==> wf_term(ob_term(#[trigger] obs[i])) && ground(ob_term(obs[i]))
            && no_dollar_var(ob_term(obs[i]))
}
...
pub open spec fn variant(o: Ob) -> Term {
    Term::Comp(ascii("variant"@), seq![o.k])
}
    ...
    decreases obs.len(),
{
    if obs.len() == 0 {
        Option::None
    } else if obs[0].heads.len() == 0 {
        Option::Some(obs[0])
    } else {
        first_empty(obs.drop_first())
    }
}

pub ghost enum Stage {
    Fail(Out),
    Obs(Seq<Ob>),
}
            ...
            s => if !payload_accepts(src_bytes(s)) {  // Missing cells never reach here (manifest_stage)
                Stage::Fail(
                    check_load(
                        Term::Comp(
                            ascii("payload_term"@),
                            seq![Term::Atom(rows[i as int].payload)],
                        ),
                    ),
                )
            } else {
                ...
                match first_empty(obs) {
                    Option::Some(o) => Stage::Fail(
                        proof_fail(
                            Term::Comp(ascii("empty_obligation"@), seq![o.docid, o.s, variant(o)]),
                        ),
                    ),
...
pub open spec fn pair(a: Term, b: Term) -> Term {
    Term::Comp(ascii("-"@), seq![a, b])
}

pub open spec fn key(o: Ob) -> Term {
    pair(pair(o.docid, o.s), o.k)
}

pub open spec fn ds(o: Ob) -> Term {
    pair(o.docid, o.s)
}
    ...
    decreases s.len(),
{
    if s.len() < 2 {
        ...
        first_dup(s.drop_first())
    }
}

pub open spec fn arg(t: Term, i: int) -> Term {
    match t {
        Term::Comp(_, a) => if 0 <= i < a.len() {
            a[i]
        } else {
            Term::Nil
        },
    ...
    decreases keys.len(),
{
    if keys.len() == 0 || arg(keys[0], 0) != d {
        if !is_sequence(ks, 1) {
            Option::Some(variant_sequence(d, ks))
        } else if keys.len() == 0 {
            Option::None
        } else {
            bad_run(keys.drop_first(), arg(keys[0], 0), seq![arg(keys[0], 1)])
        }
    } else {
        bad_run(keys.drop_first(), d, ks.push(arg(keys[0], 1)))
    }
}
...
pub open spec fn key_check(obs: Seq<Ob>) -> Option<Out> {
    ...
    match first_dup(msort(keys)) {
        Option::Some(k) => Option::Some(
            proof_fail(
                Term::Comp(
                    ascii("duplicate_obligation"@),
                    seq![
                        arg(arg(k, 0), 0),
                        arg(arg(k, 0), 1),
                        Term::Comp(ascii("variant"@), seq![arg(k, 1)]),
                    ],
                ),
            ),
        ),
...
pub open spec fn gid_name() -> Seq<u8> {
    ascii("$guideline_id"@)
}
...
{
    match t {
        Term::Comp(name, args) => (if name == gid_name() && args.len() == 5 && args[1] is Atom
            && args[2] is Int {
    ...
    decreases ts,
{
    if ts.len() == 0 {
        ...
        gid_pairs(ts[0]) + gid_pairs_all(ts.drop_first())
    }
}

pub open spec fn item_gids(it: BodyItem) -> Seq<Term> {
    match it {
        BodyItem::Pos(l) => gid_pairs(l),
        BodyItem::Naf(gs) => gid_pairs_all(gs),
    }
}
    ...
    decreases xs.len(),
{
    if xs.len() == 0 {
        Option::None
    } else if !ys.contains(xs[0]) {
        Option::Some(xs[0])
    } else {
        first_absent(xs.drop_first(), ys)
    }
}
...
pub open spec fn coverage_check(db: Seq<DocClause>, obs: Seq<Ob>) -> Option<Out> {
    ...
    match first_absent(loaded, covered) {
        Option::Some(p) => Option::Some(
            proof_fail(Term::Comp(ascii("missing_obligation"@), args_of(p))),
        ),
        Option::None => match first_absent(covered, loaded) {
            Option::Some(p) => Option::Some(
                proof_fail(Term::Comp(ascii("extra_obligation"@), args_of(p))),
            ),
            Option::None => Option::None,
        },
    }
    ...
    decreases obs.len(),
{
    if obs.len() == 0 {
        Option::None
    } else if !heads_proved(witness_db(obs[0].facts, db), obs[0].heads) {
        Option::Some(obs[0])
    } else {
        first_unproved(db, obs.drop_first())
    }
}

pub open spec fn agg_meter(rows: nat, obs: nat) -> Seq<u8> {
    ascii("ace_to_pl aggregate ok "@) + udec_bytes(rows) + ascii(" documents "@) + udec_bytes(obs)
        + ascii(" obligations\n"@)
}
...
pub open spec fn manifest_rows(mpath: Seq<u8>, manifest: Src) -> Result<Seq<MRow>, Out> {
    match manifest {
        ...
        Src::Bytes(b) => match parse_manifest(b) {
            ...
            MOut::BadLine(l) => Result::Err(
                manifest_reject(mpath, Term::Comp(ascii("line"@), seq![Term::Atom(l)])),
            ),
...
pub open spec fn manifest_stage(
    mpath: Seq<u8>,
    manifest: Src,
    pls: Seq<Src>,
    pys: Seq<Src>,
...
pub open spec fn assertions(rows: Seq<MRow>, docs: Seq<DocFile>) -> Option<Out> {
    ...
    if docs.len() != expected {
        Option::Some(
            proof_fail(
                Term::Comp(
                    ascii("document_records"@),
                    seq![Term::Int(expected as int), Term::Int(docs.len() as int)],
                ),
            ),
        )
        ...
        Option::Some(
            proof_fail(
                Term::Comp(
                    ascii("document_records"@),
                    seq![Term::Int(expected as int), Term::Int(distinct as int)],
                ),
            ),
        )
    } else {
            ...
            match key_check(obs) {
                ...
                Option::None => match coverage_check(db, obs) {
                    ...
                    Option::None => match first_unproved(db, obs) {
                        Option::Some(ob) => proof_fail(
                            Term::Comp(
                                ascii("obligation_failed"@),
                                seq![ob.docid, ob.s, variant(ob)],
                            ),
                        ),
...
pub open spec fn site(c: DocClause) -> Term {
    ...
    if gs.len() == 0 {
        ...
        Term::Comp(ascii("sentence"@), args_of(gs[0]))
    }
}
    ...
    decreases rs.len(),
{
    if rs.len() == 0 {
        Option::None
    } else if unifiable_apart(leftmost(rs[0]), rs[0].head) {
        Option::Some(rs[0])
    } else {
        first_left_recursive(rs.drop_first())
    }
}

pub open spec fn rec_meter(rows: nat, rules: nat) -> Seq<u8> {
    ascii("ace_to_pl recursion ok "@) + udec_bytes(rows) + ascii(" documents "@) + udec_bytes(rules)
        + ascii(" rule clauses\n"@)
}

pub open spec fn recursion_output(
    mpath: Seq<u8>,
    manifest: Src,
    pls: Seq<Src>,
    pys: Seq<Src>,
                ...
                Result::Ok(docs) => {
                    ...
                    match first_left_recursive(rs) {
                        Option::Some(c) => match c.head {  // heads are compounds (wf_literal)
                            Term::Comp(name, args) => proof_fail(
                                Term::Comp(
                                    ascii("left_recursive"@),
                                    seq![site(c), Term::Atom(name), Term::Int(args.len() as int)],
                                ),
                            ),
...
impl View for ESrc {
    ...
    open spec fn view(&self) -> Src {
        match self {
            ESrc::Missing => Src::Missing,
            ESrc::Bad(o) => Src::Bad(*o as nat),
            ESrc::Bytes(b) => Src::Bytes(b@),
        }
    }
}
...
impl View for ERow {
    ...
    open spec fn view(&self) -> MRow {
        MRow { pl: self.pl@, payload: self.payload@ }
    }
}
...
impl View for EOut {
    ...
    open spec fn view(&self) -> Out {
        Out { rc: self.rc as int, out: self.out@, err: self.err@ }
    }
}

pub open spec fn srcs(v: Seq<ESrc>) -> Seq<Src> {
    v.map_values(|s: ESrc| s@)
}

pub open spec fn rows_view(r: Result<Vec<ERow>, EOut>) -> Result<Seq<MRow>, Out> {
    match r {
        Result::Ok(rows) => Result::Ok(rows@.map_values(|e: ERow| e@)),
        Result::Err(o) => Result::Err(o@),
    }
}
...
pub open spec fn cells_ok(manifest: Src, pls: Seq<Src>, pys: Seq<Src>) -> bool {
    match parse_manifest(src_bytes(manifest)) {
        MOut::Rows(rows) => pls.len() == rows.len() && pys.len() == rows.len(),
        _ => true,
    }
}
