...
pub open spec fn query_reject(why: Term) -> Out {
    check_load(Term::Comp(ascii("query_file"@), seq![why]))
}
    ...
    decreases bs.len(),
{
    if bs.len() == 0 {
        ...
        bs[0].clauses.len() + clause_count(bs.drop_first())
    }
}
    ...
    decreases t,
{
    match t {
        ...
        Term::Comp(name, args) => if name == cons_name() && args.len() == 2 {
            match list_items(args[1]) {
                Option::Some(rest) => Option::Some(seq![args[0]] + rest),
...
pub open spec fn desc_ok(d: Term) -> bool {
    match d {
        Term::Comp(name, args) => (name == ascii("noun"@) && args.len() == 2 && args[0] is Atom
            && args[1] is Atom) || (name == ascii("wh"@) && args.len() == 1 && (args[0] == atom(
            "who"@,
        ) || args[0] == atom("what"@))),
        _ => false,
    }
    ...
    decreases rows.len(),
{
    if rows.len() == 0 {
        ...
        match rows[0] {
            Term::Comp(name, args) => if name == ascii("answer"@) && args.len() == 2 {
                match args[0] {
                    Term::Var(x) => if !occurs(x, goal) {
                        Option::Some(answer_var(i, "absent"@))
                    } else if seen.contains(x) {
                        Option::Some(answer_var(i, "duplicate"@))
                    } else if !desc_ok(args[1]) {
                        Option::Some(Term::Comp(ascii("answer_desc"@), seq![Term::Int(i as int)]))
                    } else {
                        rows_check(rows.drop_first(), i + 1, goal, seen.push(x))
                    },
                    _ => Option::Some(answer_var(i, "nonvar"@)),
                }
            } else {
                Option::Some(Term::Comp(ascii("answer_shape"@), seq![Term::Int(i as int)]))
            },
...
pub open spec fn sol_term(r: Seq<Term>) -> Term {
    Term::Comp(ascii("sol"@), seq![list_term(r)])
}

pub open spec fn limit() -> Term {
    Term::Comp(ascii("indeterminate"@), seq![atom("limit"@)])
}
...
pub open spec fn answer_result(db: Seq<DocClause>, q: QueryFile, rows: Seq<Term>) -> Result<
    Term,
    Out,
> {
    ...
    match solve(db, q.goal, answer_depth(), sol, rows.len() > 0, answer_inf()) {
        ...
        ROut::End { complete, rows: found } => if rows.len() == 0 {
            Result::Ok(
                if complete {
                    Term::Comp(ascii("no"@), seq![atom("finite_failure"@)])
                } else {
                    limit()
                },
            )
            ...
            Result::Err(
                proof_fail(Term::Comp(ascii("nonground_solution"@), seq![Term::Atom(q.qid)])),
            )
        } else if !complete {
            Result::Ok(limit())
        } else {
            Result::Ok(
                Term::Comp(
                    ascii("solutions"@),
                    seq![list_term(sort_unique(found.map_values(|r: Seq<Term>| sol_term(r))))],
                ),
            )
        },
    }
...
pub open spec fn answer_output(
    mpath: Seq<u8>,
    manifest: Src,
    pls: Seq<Src>,
    pys: Seq<Src>,
        ...
        Result::Ok(rows) => match custody(query) {
            ...
            Result::Ok((q, arows)) => match composition(rows, pls) {
                ...
                Result::Ok(docs) => match answer_result(db_of(docs), q, arows) {
                    Result::Err(o) => o,
                    Result::Ok(result) => ok(
                        print_answers(AnswersFile { qid: q.qid, qsha, result }),
                    ),
                },
            },
        },
