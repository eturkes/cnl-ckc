...
fn ledger(path: &Path, bundles: &Vec<EBundle>, label: &str, odb: bool) -> Result<Vec<u8>> {
    ...
    let (decisions, problem) = ckc_kernel::contract::check_ledger(&src, &known);
    if odb {
    ...
    // Earlier valid rows retain their commit checks before a later grammar error.
    if let Some(v) = problem {
        return Err(match coverage::verdict(v) {
            Err(e) => e,
