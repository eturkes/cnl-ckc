...
fn main() -> ExitCode {
    ...
    match args.get(1).map(|s| s.as_str()) {
        ...
        _ => {
            eprintln!(
                "usage: ckc trust-audit [workspace-root] | ckc align-check <align.tsv> <src.txt> <ace.txt> | ckc v1 <check|render> <pl> | ckc certify <guideline-id> | ckc certify-one <doc|query> <id> <ace> <ulex|-> <dump> <pl>"
            );
            ExitCode::from(2)
        }
    }
