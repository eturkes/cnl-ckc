...
use crate::k2_engine::{root_terms, roots_models_prefix, roots_valid};
use crate::k2_payload::EOb;
#[cfg(verus_keep_ghost)]
use crate::k2_payload::{ob_valid, ob_view, obs_prefix, obs_valid, obs_view, obs_wf_at};
use crate::k2_term::ETermArena;
#[cfg(verus_keep_ghost)]
use crate::k2_term::arena_ok;
use ckc_spec::replay::Ob;
#[cfg(verus_keep_ghost)]
use ckc_spec::replay::{ds, key, wf_obs};
#[cfg(verus_keep_ghost)]
...
proof fn pair_ground(left: ckc_spec::term::Term, right: ckc_spec::term::Term)
    requires
        ...
        ground(right),
    ensures
        ground(ckc_spec::replay::pair(left, right)),
{
...
fn ob_key_roots_inner(input_arena: ETermArena, obs: &Vec<EOb>) -> (out: (
    Vec<usize>,
    Vec<usize>,
    ETermArena,
))
    ...
    ensures
        arena_ok(&out.2),
        input_arena.nodes@.is_prefix_of(out.2.nodes@),
        roots_valid(out.2.nodes@, out.0@),
        roots_valid(out.2.nodes@, out.1@),
        root_terms(out.2.nodes@, out.0@) == obs_view(input_arena.nodes@, obs@).map_values(
            |ob: Ob| key(ob),
        ),
        root_terms(out.2.nodes@, out.1@) == obs_view(input_arena.nodes@, obs@).map_values(
            |ob: Ob| ds(ob),
        ),
        ...
        forall|i: int| 0 <= i < out.1@.len() ==> ground(out.2@[out.1@[i] as int]),
{
    ...
    while i < obs.len()
        invariant
            ...
            obs_valid(arena.nodes@, obs@),
            models == obs_view(origin, obs@),
            models == obs_view(arena.nodes@, obs@),
            wf_obs(models),
            i <= obs.len(),
            keys.len() == i,
            pairs.len() == i,
            roots_valid(arena.nodes@, keys@),
            roots_valid(arena.nodes@, pairs@),
            forall|j: int|
                0 <= j < i ==> arena@[keys@[j] as int] == key(models[j]) && ground(
                    arena@[keys@[j] as int],
                ),
            forall|j: int|
                0 <= j < i ==> arena@[pairs@[j] as int] == ds(models[j]) && ground(
                    arena@[pairs@[j] as int],
                ),
        ...
        decreases obs.len() - i,
    {
        ...
        pairs.push(pair);
        proof {
            assert forall|j: int| 0 <= j < keys.len() implies arena@[keys@[j] as int]
                == ckc_spec::replay::key(models[j]) && ground(arena@[keys@[j] as int]) by {
                if j < before_keys.len() {
                    assert(keys@[j] == before_keys[j]);
                }
            }
            assert forall|j: int| 0 <= j < pairs.len() implies arena@[pairs@[j] as int] == ds(
                models[j],
            ) && ground(arena@[pairs@[j] as int]) by {
                if j < before_pairs.len() {
                    assert(pairs@[j] == before_pairs[j]);
                }
            }
        }
        i += 1;
...
pub fn ob_key_roots(arena: &mut ETermArena, obs: &Vec<EOb>) -> (out: (Vec<usize>, Vec<usize>))
    requires
        arena_ok(old(arena)),
        obs_valid(old(arena).nodes@, obs@),
        wf_obs(obs_view(old(arena).nodes@, obs@)),
    ensures
        arena_ok(final(arena)),
        old(arena).nodes@.is_prefix_of(final(arena).nodes@),
        roots_valid(final(arena).nodes@, out.0@),
        roots_valid(final(arena).nodes@, out.1@),
        root_terms(final(arena).nodes@, out.0@) == obs_view(old(arena).nodes@, obs@).map_values(
            |ob: Ob| key(ob),
        ),
        root_terms(final(arena).nodes@, out.1@) == obs_view(old(arena).nodes@, obs@).map_values(
            |ob: Ob| ds(ob),
        ),
...
pub open spec fn selected_root(nodes: Seq<crate::k2_term::ENode>, out: Option<usize>) -> Option<
    ckc_spec::term::Term,
> {
    match out {
        Some(root) => Some(nodes[root as int].term@),
        None => None,
    }
}
...
pub proof fn sort_views(arena: &ETermArena, roots: Seq<usize>)
    ...
    ensures
        crate::k2_sort::root_terms(arena, roots) == root_terms(arena.nodes@, roots),
{
...
fn first_dup_root(arena: &ETermArena, roots: &Vec<usize>) -> (out: Option<usize>)
    requires
        crate::k2_sort::roots_ok(arena, roots@),
        crate::k2_sort::roots_ground(arena, roots@),
    ensures
        out matches Some(root) ==> root < arena.nodes.len(),
        selected_root(arena.nodes@, out) == ckc_spec::replay::first_dup(
        ...
        ),
{
    ...
    while roots.len() - i >= 2
        invariant
            ...
            ckc_spec::replay::first_dup(models) == ckc_spec::replay::first_dup(
        ...
        decreases roots.len() - i,
    {
        ...
        if crate::k2_sort::term_equal(arena, roots[i], roots[i + 1]) {
            return Some(roots[i]);
        }
        i += 1;
    }
...
fn sequence_inner(input_arena: ETermArena, roots: &Vec<usize>) -> (out: (bool, ETermArena))
    requires
        crate::k2_sort::roots_ok(&input_arena, roots@),
        ...
        out.0 == ckc_spec::replay::is_sequence(root_terms(input_arena.nodes@, roots@), 1),
{
    ...
    while i < roots.len()
        invariant
            ...
            crate::k2_sort::roots_ground(&arena, roots@),
            i <= roots.len(),
            models == root_terms(origin, roots@),
            models == root_terms(arena.nodes@, roots@),
            ckc_spec::replay::is_sequence(models, 1) == ckc_spec::replay::is_sequence(
                models.skip(i as int),
                i as int + 1,
            ),
        decreases roots.len() - i,
    {
        ...
        if !crate::k2_sort::term_equal(&arena, roots[i], expected) {
            return (false, arena);
        }
        i += 1;
    }
...
fn sequence_exec(arena: &mut ETermArena, roots: &Vec<usize>) -> (out: bool)
    requires
        crate::k2_sort::roots_ok(old(arena), roots@),
        crate::k2_sort::roots_ground(old(arena), roots@),
    ensures
        arena_ok(final(arena)),
        old(arena).nodes@.is_prefix_of(final(arena).nodes@),
...
fn variant_sequence_root(arena: &mut ETermArena, d: usize, variants: &Vec<usize>) -> (out: usize)
    requires
        crate::k2_term::root_ok(old(arena), d),
        roots_valid(old(arena).nodes@, variants@),
    ensures
        arena_ok(final(arena)),
        old(arena).nodes@.is_prefix_of(final(arena).nodes@),
        ...
        final(arena)@[out as int] == ckc_spec::replay::variant_sequence(
            old(arena)@[d as int],
            root_terms(old(arena).nodes@, variants@),
        ),
{
    ...
    proof {
        roots_models_prefix(origin, arena.nodes@, variants@);
    }
    let list = crate::k2_walk::list_root(arena, variants);
    let name: &[u8] = b"variant_sequence";
    proof {
        reveal_byteslit(b"variant_sequence");
        reveal_strlit("variant_sequence");
        reveal(ckc_spec::v1text::ascii);
...
fn bad_run_inner(input_arena: ETermArena, keys: &Vec<usize>) -> (out: (Option<usize>, ETermArena))
    requires
        crate::k2_sort::roots_ok(&input_arena, keys@),
        crate::k2_sort::roots_ground(&input_arena, keys@),
    ensures
        arena_ok(&out.1),
        input_arena.nodes@.is_prefix_of(out.1.nodes@),
        out.0 matches Some(root) ==> root < out.1.nodes.len(),
        selected_root(out.1.nodes@, out.0) == ckc_spec::replay::first_bad_run(
            root_terms(input_arena.nodes@, keys@),
        ),
{
    ...
    while i < keys.len()
        invariant
            ...
            origin == input_arena.nodes@,
            origin.is_prefix_of(arena.nodes@),
            crate::k2_sort::roots_ok(&arena, keys@),
            crate::k2_sort::roots_ground(&arena, keys@),
            crate::k2_sort::roots_ok(&arena, variants@),
            crate::k2_sort::roots_ground(&arena, variants@),
            d < arena.nodes.len(),
            ground(arena@[d as int]),
            1 <= i <= keys.len(),
            models == root_terms(origin, keys@),
            models == root_terms(arena.nodes@, keys@),
            ckc_spec::replay::first_bad_run(models) == ckc_spec::replay::bad_run(
                models.skip(i as int),
                arena@[d as int],
                root_terms(arena.nodes@, variants@),
            ),
        decreases keys.len() - i,
    {
        ...
        if !crate::k2_sort::term_equal(&arena, d, next_d) {
            ...
            if !sequential {
                ...
                proof {
                    crate::k2_load::prefix_chain(origin, before_detail, arena.nodes@);
                }
            ...
            variants = Vec::new();
            proof {
                assert_seqs_equal!(root_terms(arena.nodes@, variants@) == Seq::empty());
            }
        }
        ...
        proof {
            ...
            assert forall|j: int| 0 <= j < variants.len() implies variants@[j] < arena.nodes.len()
                && ground(arena@[variants@[j] as int]) by {
                if j < previous_variants.len() {
                    assert(variants@[j] == previous_variants[j]);
                }
            }
...
fn first_bad_run_exec(arena: &mut ETermArena, keys: &Vec<usize>) -> (out: Option<usize>)
    requires
        crate::k2_sort::roots_ok(old(arena), keys@),
        crate::k2_sort::roots_ground(old(arena), keys@),
    ensures
        arena_ok(final(arena)),
        old(arena).nodes@.is_prefix_of(final(arena).nodes@),
        out matches Some(root) ==> root < final(arena).nodes.len(),
        selected_root(final(arena).nodes@, out) == ckc_spec::replay::first_bad_run(
            root_terms(old(arena).nodes@, keys@),
        ),
{
    let mut owned = crate::k2_reject::empty_arena();
    core::mem::swap(arena, &mut owned);
...
fn duplicate_error(arena: &mut ETermArena, key: usize) -> (out: ckc_spec::replay::EOut)
    requires
        crate::k2_term::root_ok(old(arena), key),
    ensures
        arena_ok(final(arena)),
        old(arena).nodes@.is_prefix_of(final(arena).nodes@),
        out@ == ckc_spec::replay::proof_fail(
            ckc_spec::term::Term::Comp(
                ckc_spec::v1text::ascii("duplicate_obligation"@),
                seq![
        ...
        ),
{
    ...
    proof {
        ...
        reveal_byteslit(b"duplicate_obligation");
        reveal_strlit("duplicate_obligation");
        reveal(ckc_spec::v1text::ascii);
...
pub fn key_check_exec(arena: &mut ETermArena, obs: &Vec<EOb>) -> (out: (
    Option<ckc_spec::replay::EOut>,
    Vec<usize>,
))
    requires
        arena_ok(old(arena)),
    ...
    ensures
        ...
        old(arena).nodes@.is_prefix_of(final(arena).nodes@),
        roots_valid(final(arena).nodes@, out.1@),
        crate::k2_sort::roots_ground(final(arena), out.1@),
        root_terms(final(arena).nodes@, out.1@) == obs_view(old(arena).nodes@, obs@).map_values(
            |ob: Ob| ds(ob),
        ),
        crate::k2_output::option_out_view(out.0) == ckc_spec::replay::key_check(
            obs_view(old(arena).nodes@, obs@),
        ),
{
    let ghost origin = arena.nodes@;
    ...
    proof {
        sort_views(arena, keys@);
    }
    let sorted = crate::k2_sort::msort(arena, &keys);
    proof {
        sort_views(arena, sorted@);
    }
...
fn contains_root(arena: &ETermArena, roots: &Vec<usize>, root: usize) -> (out: bool)
    requires
        crate::k2_sort::roots_ok(arena, roots@),
        crate::k2_sort::roots_ground(arena, roots@),
        crate::k2_term::root_ok(arena, root),
        ground(arena@[root as int]),
    ensures
        out == root_terms(arena.nodes@, roots@).contains(arena@[root as int]),
{
    let mut i = 0usize;
    while i < roots.len()
        invariant
            ...
            i <= roots.len(),
            forall|j: int|
                0 <= j < i ==> #[trigger] root_terms(arena.nodes@, roots@)[j]
                    != arena@[root as int],
        decreases roots.len() - i,
    {
        if crate::k2_sort::term_equal(arena, roots[i], root) {
            proof {
                assert(root_terms(arena.nodes@, roots@)[i as int] == arena@[root as int]);
            }
            return true;
        }
        i += 1;
...
fn first_absent_root(arena: &ETermArena, xs: &Vec<usize>, ys: &Vec<usize>) -> (out: Option<usize>)
    requires
        crate::k2_sort::roots_ok(arena, xs@),
        crate::k2_sort::roots_ground(arena, xs@),
        crate::k2_sort::roots_ok(arena, ys@),
        crate::k2_sort::roots_ground(arena, ys@),
    ensures
        out matches Some(root) ==> root < arena.nodes.len() && xs@.contains(root),
        selected_root(arena.nodes@, out) == ckc_spec::replay::first_absent(
            root_terms(arena.nodes@, xs@),
            root_terms(arena.nodes@, ys@),
        ),
{
    ...
    while i < xs.len()
        invariant
            ...
            crate::k2_sort::roots_ground(arena, ys@),
            i <= xs.len(),
            models == root_terms(arena.nodes@, xs@),
            others == root_terms(arena.nodes@, ys@),
            ckc_spec::replay::first_absent(models, others) == ckc_spec::replay::first_absent(
                models.skip(i as int),
                others,
            ),
        decreases xs.len() - i,
    {
        ...
        if !contains_root(arena, ys, xs[i]) {
            proof {
                assert(xs@.contains(xs@[i as int]));
            }
            return Some(xs[i]);
        }
        i += 1;
...
fn loaded_pairs_inner(input_arena: ETermArena, db: &Vec<crate::k2_engine::EClause>) -> (out: (
    Vec<usize>,
    ETermArena,
))
    requires
        arena_ok(&input_arena),
        crate::k2_engine::db_valid(input_arena.nodes@, db@),
    ensures
        arena_ok(&out.1),
        input_arena.nodes@.is_prefix_of(out.1.nodes@),
        roots_valid(out.1.nodes@, out.0@),
        forall|i: int|
            0 <= i < out.0.len() ==> ground(out.1@[out.0@[i] as int]) && ckc_spec::engine::args_of(
                out.1@[out.0@[i] as int],
            ).len() == 2,
        root_terms(out.1.nodes@, out.0@) == crate::k2_engine::db_view(
            input_arena.nodes@,
            db@,
        ).map_values(|c: ckc_spec::v1text::DocClause| ckc_spec::replay::clause_gids(c)).flatten(),
{
    ...
    let ghost parts = models.map_values(
        |c: ckc_spec::v1text::DocClause| ckc_spec::replay::clause_gids(c),
    );
    let mut arena = input_arena;
    let mut out = Vec::new();
    let mut i = 0usize;
    ...
    while i < db.len()
        invariant
            ...
            origin == input_arena.nodes@,
            origin.is_prefix_of(arena.nodes@),
            crate::k2_engine::db_valid(arena.nodes@, db@),
            models == crate::k2_engine::db_view(origin, db@),
            models == crate::k2_engine::db_view(arena.nodes@, db@),
            parts == models.map_values(
                |c: ckc_spec::v1text::DocClause| ckc_spec::replay::clause_gids(c),
            ),
            i <= db.len(),
            ...
            forall|j: int|
                0 <= j < out.len() ==> ground(arena@[out@[j] as int]) && ckc_spec::engine::args_of(
                    arena@[out@[j] as int],
                ).len() == 2,
        decreases db.len() - i,
    {
        ...
        proof {
            assert forall|j: int| 0 <= j < out.len() implies out@[j] < arena.nodes.len() && ground(
                arena@[out@[j] as int],
            ) && ckc_spec::engine::args_of(arena@[out@[j] as int]).len() == 2 by {
                if j < left.len() {
                    assert(out@[j] == left[j]);
                } else {
...
fn loaded_pairs_exec(arena: &mut ETermArena, db: &Vec<crate::k2_engine::EClause>) -> (out: Vec<
    usize,
>)
    requires
        arena_ok(old(arena)),
    ...
    ensures
        ...
        forall|i: int|
            0 <= i < out.len() ==> ground(final(arena)@[out@[i] as int])
                && ckc_spec::engine::args_of(final(arena)@[out@[i] as int]).len() == 2,
        root_terms(final(arena).nodes@, out@) == crate::k2_engine::db_view(
            old(arena).nodes@,
            db@,
        ).map_values(|c: ckc_spec::v1text::DocClause| ckc_spec::replay::clause_gids(c)).flatten(),
{
...
fn pair_error(arena: &mut ETermArena, pair: usize, name: &[u8]) -> (out: ckc_spec::replay::EOut)
    requires
        crate::k2_term::root_ok(old(arena), pair),
        ckc_spec::engine::args_of(old(arena)@[pair as int]).len() == 2,
    ensures
        arena_ok(final(arena)),
        old(arena).nodes@.is_prefix_of(final(arena).nodes@),
        out@ == ckc_spec::replay::proof_fail(
            ckc_spec::term::Term::Comp(name@, ckc_spec::engine::args_of(old(arena)@[pair as int])),
        ),
{
...
pub fn coverage_check_exec(
    arena: &mut ETermArena,
    db: &Vec<crate::k2_engine::EClause>,
    pairs: &Vec<usize>,
    Ghost(obs): Ghost<Seq<Ob>>,
) -> (out: Option<ckc_spec::replay::EOut>)
    requires
        arena_ok(old(arena)),
        crate::k2_engine::db_valid(old(arena).nodes@, db@),
        roots_valid(old(arena).nodes@, pairs@),
        crate::k2_sort::roots_ground(old(arena), pairs@),
        root_terms(old(arena).nodes@, pairs@) == obs.map_values(|ob: Ob| ds(ob)),
    ensures
        arena_ok(final(arena)),
        old(arena).nodes@.is_prefix_of(final(arena).nodes@),
        crate::k2_output::option_out_view(out) == ckc_spec::replay::coverage_check(
        ...
        ),
{
    ...
    proof {
        ...
        sort_views(arena, all@);
        sort_views(arena, pairs@);
    }
    let loaded = crate::k2_sort::sort_unique(arena, &all);
    let covered = crate::k2_sort::sort_unique(arena, pairs);
    proof {
        sort_views(arena, loaded@);
        sort_views(arena, covered@);
        let shape = |t: ckc_spec::term::Term| ckc_spec::engine::args_of(t).len() == 2;
        assert forall|i: int| 0 <= i < all.len() implies shape(
            #[trigger] root_terms(arena.nodes@, all@)[i],
        ) by {
            assert(root_terms(arena.nodes@, all@)[i] == arena@[all@[i] as int]);
        }
        assert forall|i: int| 0 <= i < pairs.len() implies shape(
            #[trigger] root_terms(arena.nodes@, pairs@)[i],
        ) by {
    ...
    proof {
        reveal_byteslit(b"missing_obligation");
        reveal_strlit("missing_obligation");
        reveal_byteslit(b"extra_obligation");
        reveal_strlit("extra_obligation");
        reveal(ckc_spec::v1text::ascii);
    ...
    if let Some(root) = first_absent_root(arena, &loaded, &covered) {
        ...
        proof {
            crate::k2_load::prefix_chain(origin, before_error, arena.nodes@);
        }
        return Some(error);
    }
...
fn unproved_inner(
    ...
    ensures
        ...
        crate::k2_payload::selected_ob(out.1.nodes@, obs@, out.0)
            == ckc_spec::replay::first_unproved(
            crate::k2_engine::db_view(input_arena.nodes@, db@),
            obs_view(input_arena.nodes@, obs@),
        ),
{
    ...
    while i < obs.len()
        invariant
            ...
            crate::k2_engine::db_valid(arena.nodes@, db@),
            obs_valid(arena.nodes@, obs@),
            database == crate::k2_engine::db_view(origin, db@),
            database == crate::k2_engine::db_view(arena.nodes@, db@),
            models == obs_view(origin, obs@),
            models == obs_view(arena.nodes@, obs@),
            i <= obs.len(),
            ckc_spec::replay::first_unproved(database, models) == ckc_spec::replay::first_unproved(
                database,
                models.skip(i as int),
            ),
        decreases obs.len() - i,
...
pub fn first_unproved_exec(
    ...
    ensures
        ...
        crate::k2_payload::selected_ob(final(arena).nodes@, obs@, out)
            == ckc_spec::replay::first_unproved(
            crate::k2_engine::db_view(old(arena).nodes@, db@),
            obs_view(old(arena).nodes@, obs@),
        ),
