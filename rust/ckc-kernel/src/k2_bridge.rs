...
use crate::k2_engine::{
    body_item_valid, body_item_view, body_items_valid, body_items_view,
    clause_valid, clause_view, db_valid, db_view, root_terms, roots_valid,
};
use crate::v1_term_impl::{EBodyRoot, EDocClause, EParsedV1};
use crate::k2_output::{atom_root, int_root, comp1, comp3};
#[cfg(verus_keep_ghost)]
use crate::v1_term_impl::{
    body_root_elim, body_root_ok, body_roots_elim, doc_clause_roots_elim,
    doc_clause_roots_ok, doc_clauses_roots_elim, doc_clauses_roots_ok,
};
        ...
        clause_view(arena.nodes@, &out) == ckc_spec::engine::fact_clause(arena@[head as int]),
{
    ...
    proof { assert_seqs_equal!(body_items_view(arena.nodes@, out.body@) == Seq::empty()); }
    out
}

pub fn append_db(arena: &ETermArena, target: &mut Vec<EClause>, mut rest: Vec<EClause>)
    requires db_valid(arena.nodes@, old(target)@), db_valid(arena.nodes@, rest@),
    ensures
        db_valid(arena.nodes@, final(target)@),
        db_view(arena.nodes@, final(target)@)
            == db_view(arena.nodes@, old(target)@) + db_view(arena.nodes@, rest@),
{
    ...
    proof {
        assert forall|i: int| 0 <= i < target@.len()
            implies #[trigger] clause_valid(arena.nodes@, &target@[i]) by {
...
fn body_from_parser(arena: &ETermArena, root: &EBodyRoot, Ghost(item): Ghost<BodyItem>)
    -> (out: EBodyItem)
    requires body_root_ok(arena.nodes@, root, item), wf_body_item(item),
    ensures body_item_valid(arena.nodes@, &out), body_item_view(arena.nodes@, &out) == item,
{
    proof { body_root_elim(arena.nodes@, root, item); }
    match root {
        EBodyRoot::Pos(index) => {
            proof {
                assert(item is Pos);
        ...
        EBodyRoot::Naf(roots) => {
            proof {
                assert(item is Naf);
                let terms = match item { BodyItem::Naf(terms) => terms, _ => Seq::empty() };
                reveal(wf_body_item);
                assert(roots@.len() > 0);
                assert_seqs_equal!(root_terms(arena.nodes@, roots@) == terms);
                assert forall|j: int| 0 <= j < roots@.len() implies roots@[j] < arena.nodes@.len() by {
                    assert(arena.nodes@[roots@[j] as int].term@ == terms[j]);
                }
            }
            let out = EBodyItem::Naf { roots: roots.clone() };
            proof { assert(body_item_valid(arena.nodes@, &out)); }
            out
        },
    }
}
...
pub fn clause_from_parser(arena: &ETermArena, clause: &EDocClause) -> (out: EClause)
    requires doc_clause_roots_ok(arena.nodes@, clause), wf_clause(clause@),
    ensures clause_valid(arena.nodes@, &out), clause_view(arena.nodes@, &out) == clause@,
{
    proof {
    ...
    while i < clause.body.len()
        invariant
            i <= clause.body.len(), body@.len() == i,
            doc_clause_roots_ok(arena.nodes@, clause),
            clause.body@.len() == clause@.body.len(),
            forall|j: int| 0 <= j < clause.body@.len()
                ==> #[trigger] body_root_ok(arena.nodes@, &clause.body@[j], clause@.body[j]),
            wf_clause(clause@),
        ...
        decreases clause.body.len() - i,
    {
        ...
        body.push(item);
        proof {
            assert forall|j: int| 0 <= j < body@.len()
                implies #[trigger] body_item_valid(arena.nodes@, &body@[j]) by {
    ...
    proof { assert(clause_view(arena.nodes@, &out) == clause@); }
    out
}

pub fn clauses_from_parser(
    arena: &ETermArena, clauses: &Vec<EDocClause>, Ghost(models): Ghost<Seq<DocClause>>,
) -> (out: Vec<EClause>)
    requires
        doc_clauses_roots_ok(arena.nodes@, clauses@, models),
        forall|i: int| 0 <= i < models.len() ==> wf_clause(#[trigger] models[i]),
    ensures db_valid(arena.nodes@, out@), db_view(arena.nodes@, out@) == models,
{
    proof { doc_clauses_roots_elim(arena.nodes@, clauses@, models); }
    let mut out = Vec::new();
    let mut i = 0usize;
    while i < clauses.len()
        invariant
            i <= clauses.len(), out@.len() == i, clauses@.len() == models.len(),
            forall|j: int| 0 <= j < clauses@.len() ==> (#[trigger] clauses@[j])@ == models[j],
            forall|j: int| 0 <= j < clauses@.len()
                ==> #[trigger] doc_clause_roots_ok(arena.nodes@, &clauses@[j]),
            forall|j: int| 0 <= j < models.len() ==> wf_clause(#[trigger] models[j]),
            db_valid(arena.nodes@, out@), db_view(arena.nodes@, out@) == models.take(i as int),
        decreases clauses.len() - i,
    {
        ...
        proof {
            assert forall|j: int| 0 <= j < out@.len()
                implies #[trigger] clause_valid(arena.nodes@, &out@[j]) by {
...
proof fn bundle_clauses_wf(bundles: Seq<ckc_spec::v1text::Bundle>)
    requires forall|i: int| 0 <= i < bundles.len() ==> ckc_spec::v1text::wf_bundle(#[trigger] bundles[i]),
    ensures forall|i: int| 0 <= i < crate::v1_term_impl::doc_clause_models(bundles).len()
        ==> wf_clause(#[trigger] crate::v1_term_impl::doc_clause_models(bundles)[i]),
    decreases bundles.len(),
{
    ...
    if bundles.len() > 0 {
        ...
        assert forall|i: int| 0 <= i < rest.len()
            implies ckc_spec::v1text::wf_bundle(#[trigger] rest[i]) by {
        ...
        assert forall|i: int| 0 <= i < parts.flatten().len()
            implies wf_clause(#[trigger] parts.flatten()[i]) by {
...
fn document_record_root(
    arena: &mut ETermArena, parsed: &EParsedV1,
        ...
        final(arena)@[root as int] == ckc_spec::v1text::doc_record_term(doc),
{
    ...
    proof {
        reveal_byteslit(b"none"); reveal_strlit("none");
        reveal_byteslit(b"sha256"); reveal_strlit("sha256");
        reveal_byteslit(b"ace_sha256"); reveal_strlit("ace_sha256");
        reveal_byteslit(b"ulex"); reveal_strlit("ulex");
        ...
        db_view(final(arena).nodes@, out@) == ckc_spec::replay::doc_db(doc),
{
    ...
    let clauses = clauses_from_parser(arena, &parsed.clauses,
...
fn copy_clause(arena: &ETermArena, clause: &EClause) -> (out: EClause)
    requires clause_valid(arena.nodes@, clause),
    ensures clause_valid(arena.nodes@, &out), clause_view(arena.nodes@, &out) == clause_view(arena.nodes@, clause),
{
    let mut body = Vec::new();
    let mut i = 0usize;
    while i < clause.body.len()
        invariant clause_valid(arena.nodes@, clause), i <= clause.body.len(), body.len() == i,
            crate::k2_engine::body_items_valid(arena.nodes@, body@),
            forall|j: int| 0 <= j < i ==> crate::k2_engine::body_item_view(arena.nodes@, &body@[j])
                == crate::k2_engine::body_item_view(arena.nodes@, &clause.body@[j]),
        decreases clause.body.len() - i,
    {
        let ghost before = body@;
        proof { assert(crate::k2_engine::body_item_valid(arena.nodes@, &clause.body@[i as int])); }
        ...
        body.push(item);
        proof {
            assert forall|j: int| 0 <= j < body.len() implies
                crate::k2_engine::body_item_valid(arena.nodes@, &body@[j])
    ...
    proof { assert_seqs_equal!(body_items_view(arena.nodes@, body@) == body_items_view(arena.nodes@, clause.body@)); }
    EClause { head: clause.head, body }
}

pub fn witness_db_exec(arena: &ETermArena, facts: &Vec<usize>, db: &Vec<EClause>) -> (out: Vec<EClause>)
    requires crate::k2_term::arena_ok(arena), roots_valid(arena.nodes@, facts@), db_valid(arena.nodes@, db@),
    ensures db_valid(arena.nodes@, out@), db_view(arena.nodes@, out@)
        == ckc_spec::engine::witness_db(root_terms(arena.nodes@, facts@), db_view(arena.nodes@, db@)),
{
    let ghost models = root_terms(arena.nodes@, facts@).reverse().map_values(|t: ckc_spec::term::Term| ckc_spec::engine::fact_clause(t));
    let mut out = Vec::new();
    let mut i = 0usize;
    proof {
    ...
    while i < facts.len()
        invariant crate::k2_term::arena_ok(arena), roots_valid(arena.nodes@, facts@),
            models == root_terms(arena.nodes@, facts@).reverse().map_values(|t: ckc_spec::term::Term| ckc_spec::engine::fact_clause(t)),
            i <= facts.len(), out.len() == i, db_valid(arena.nodes@, out@),
        ...
        decreases facts.len() - i,
    {
        ...
        out.push(fact);
        proof {
            assert forall|j: int| 0 <= j < out.len() implies clause_valid(arena.nodes@, &out@[j]) by {
                if j < before.len() { assert(out@[j] == before[j]); }
            }
    ...
    while j < db.len()
        invariant db_valid(arena.nodes@, db@), j <= db.len(), db_valid(arena.nodes@, out@),
    ...
    {
        let ghost before = out@;
        proof { assert(clause_valid(arena.nodes@, &db@[j as int])); }
        let clause = copy_clause(arena, &db[j]);
        out.push(clause);
        proof {
            assert forall|k: int| 0 <= k < out.len() implies clause_valid(arena.nodes@, &out@[k]) by {
    ...
    proof { assert_seqs_equal!(db_view(arena.nodes@, db@).take(j as int) == db_view(arena.nodes@, db@)); }
    out
}
