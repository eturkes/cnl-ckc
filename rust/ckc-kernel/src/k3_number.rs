...
pub open spec fn keys_view(keys: Seq<usize>) -> Seq<nat> {
    keys.map_values(|k: usize| k as nat)
}

proof fn streams_concat(left: Seq<Term>, right: Seq<Term>)
    ensures
        var_stream_all(left + right) == var_stream_all(left) + var_stream_all(right),
    decreases left.len(),
{
    if left.len() > 0 {
...
proof fn contains_cons(head: nat, tail: Seq<nat>, key: nat)
    ensures
        (seq![head] + tail).contains(key) == (head == key || tail.contains(key)),
{
    vstd::seq_lib::lemma_seq_concat_contains_all_elements(seq![head], tail, key);
    if head == key {
        assert(seq![head][0] == key);
    } else {
        assert forall|i: int| 0 <= i < seq![head].len() implies seq![head][i] != key by {}
    }
}
    ...
    if values.len() == 0 {
        ...
        if seen.contains(head) {
            ...
            if key == head {
                ...
                vstd::set::lemma_set_insert_different(seen, key, head);
            }
        }
        reveal(firsts);
    }
    ...
    if values.len() > 0 {
        ...
        if seen.contains(head) {
            ...
            assert forall|i: int, j: int|
                0 <= i < seq![head].len() && 0 <= j < rest.len() implies seq![head][i]
                != rest[j] by {
                rest.lemma_index_contains(j);
            }
...
proof fn stream_cons(head: Term, tail: Seq<Term>)
    ensures
        var_stream_all(seq![head] + tail) == var_stream(head) + var_stream_all(tail),
{
...
proof fn firsts_cons(head: nat, tail: Seq<nat>, seen: Set<nat>)
    ensures
        firsts(seq![head] + tail, seen) == if seen.contains(head) {
            firsts(tail, seen)
        } else {
            seq![head] + firsts(tail, seen.insert(head))
        },
{
    assert((seq![head] + tail).len() > 0);
    assert((seq![head] + tail)[0] == head);
...
fn position(keys: &Vec<usize>, key: usize) -> (out: usize)
    ensures
        out <= keys.len(),
        out as nat == pos_of(keys_view(keys@), key as nat),
        out < keys.len() ==> keys@[out as int] == key,
        out == keys.len() <==> !keys@.contains(key),
{
    let mut i = 0usize;
    proof {
        assert_seqs_equal!(keys@.skip(0) == keys@);
    }
    while i < keys.len()
        invariant
            i <= keys.len(),
            forall|j: int| 0 <= j < i ==> keys@[j] != key,
            pos_of(keys_view(keys@), key as nat) == i as nat + pos_of(
                keys_view(keys@.skip(i as int)),
                key as nat,
            ),
    ...
    proof {
        assert_seqs_equal!(keys_view(keys@.skip(i as int)) == Seq::empty());
        reveal(pos_of);
    }
    i
}
...
proof fn work_terms(nodes: Seq<ENode>, roots: Seq<usize>)
    ensures
        roots_work(nodes, roots) == crate::k2_engine::terms_size(root_terms(nodes, roots)),
    decreases roots.len(),
{
    if roots.len() > 0 {
...
fn visit(arena: &ETermArena, root: usize) -> (out: EVisit)
    ...
    ensures
        match out {
            ...
            EVisit::Children(rs) => roots_valid(arena.nodes@, rs@) && var_stream(
                arena@[root as int],
            ) == var_stream_all(root_terms(arena.nodes@, rs@)) && crate::k2_engine::term_size(
                arena@[root as int],
            ) == 1 + roots_work(arena.nodes@, rs@),
...
fn append_key(input: Vec<usize>, key: usize, Ghost(tail): Ghost<Seq<nat>>) -> (out: Vec<usize>)
    ensures
        keys_view(input@) + firsts(seq![key as nat] + tail, keys_view(input@).to_set())
            == keys_view(out@) + firsts(tail, keys_view(out@).to_set()),
{
    ...
    proof {
        firsts_cons(key as nat, tail, keys_view(keys@).to_set());
    }
    if found == keys.len() {
        proof {
...
fn collect_step(arena: &ETermArena, input_tasks: Vec<usize>, input_keys: Vec<usize>) -> (out: (
    Vec<usize>,
    Vec<usize>,
))
    requires
        arena_ok(arena),
        roots_valid(arena.nodes@, input_tasks@),
        input_tasks.len() > 0,
    ensures
        roots_valid(arena.nodes@, out.0@),
        roots_work(arena.nodes@, out.0@) < roots_work(arena.nodes@, input_tasks@),
        keys_view(input_keys@) + firsts(
            var_stream_all(root_terms(arena.nodes@, input_tasks@)),
            keys_view(input_keys@).to_set(),
        ) == keys_view(out.1@) + firsts(
            var_stream_all(root_terms(arena.nodes@, out.0@)),
            keys_view(out.1@).to_set(),
        ),
{
    hide(firsts);
    hide(var_stream_all);
    ...
    match visit(arena, current) {
        ...
        EVisit::Children(mut children) => {
            ...
            proof {
                streams_concat(
                    root_terms(arena.nodes@, children@),
                    root_terms(arena.nodes@, tasks@),
                );
            ...
            proof {
                ...
                assert(var_stream_all(root_terms(arena.nodes@, tasks@)) == var_stream_all(
                    root_terms(arena.nodes@, old_tasks),
                ));
            }
        },
        EVisit::Scalar => {},
...
fn collect(arena: &ETermArena, root: usize) -> (out: Vec<usize>)
    requires
        root_ok(arena, root),
    ensures
        keys_view(out@) == firsts(var_stream(arena@[root as int]), Set::empty()),
{
    hide(firsts);
    hide(var_stream_all);
    hide(var_stream);
    ...
    while tasks.len() > 0
        invariant
            ...
            firsts(var_stream(model), Set::empty()) == keys_view(keys@) + firsts(
                var_stream_all(root_terms(arena.nodes@, tasks@)),
                keys_view(keys@).to_set(),
            ),
...
{
    match t {
        Term::Var(k) => if pos_of(keys, k) < count {
            dollar_var(base + pos_of(keys, k))
        } else {
            t
        },
...
proof fn dollar_subst(n: nat, key: nat, value: Term)
    ensures
        ckc_spec::engine::subst(dollar_var(n), key, value) == dollar_var(n),
{
    reveal(dollar_var);
...
proof fn partial_step(t: Term, keys: Seq<nat>, base: nat, i: nat)
    requires
        ...
        i < keys.len(),
    ensures
        ckc_spec::engine::subst(partial(t, keys, base, i), keys[i as int], dollar_var(base + i))
            == partial(t, keys, base, i + 1),
    decreases t,
{
    match t {
        Term::Var(k) => {
            ...
            pos_member(keys, k);
            if pos_of(keys, k) < i {
                dollar_subst(base + pos_of(keys, k), keys[i as int], dollar_var(base + i));
            }
            reveal(partial);
...
proof fn partial_all_step(ts: Seq<Term>, keys: Seq<nat>, base: nat, i: nat)
    ...
    ensures
        ckc_spec::engine::subst_all(
            partial_all(ts, keys, base, i),
            keys[i as int],
            dollar_var(base + i),
        ) == partial_all(ts, keys, base, i + 1),
    decreases ts,
{
    if ts.len() > 0 {
    ...
    reveal_with_fuel(partial_all, 2);
    reveal_with_fuel(ckc_spec::engine::subst_all, 2);
}

proof fn partial_done(t: Term, keys: Seq<nat>, base: nat)
    requires
        var_stream(t).to_set().subset_of(keys.to_set()),
    ensures
        partial(t, keys, base, keys.len()) == number_with(t, keys, base),
    decreases t,
{
    match t {
        Term::Var(k) => {
...
proof fn partial_all_done(ts: Seq<Term>, keys: Seq<nat>, base: nat)
    requires
        var_stream_all(ts).to_set().subset_of(keys.to_set()),
    ensures
        partial_all(ts, keys, base, keys.len()) == number_all(ts, keys, base),
    decreases ts,
{
    if ts.len() > 0 {
        reveal(var_stream_all);
    ...
    reveal_with_fuel(partial_all, 2);
    reveal_with_fuel(number_all, 2);
}

fn number_inner(mut arena: ETermArena, root: usize, base: usize) -> (out: (
    ETermArena,
    usize,
    usize,
))
        ...
        (out.0@[out.1 as int], out.2 as nat) == number(arena@[root as int], base as nat),
{
    ...
    while i < keys.len()
        invariant
            ...
            root < origin.len(),
            model == origin[root as int].term@,
            fs == keys_view(keys@),
            fs == firsts(var_stream(model), Set::empty()),
            fs.no_duplicates(),
            fs.to_set() == var_stream(model).to_set(),
            i <= keys.len(),
            next as nat == base as nat + i as nat,
            next <= arena.nodes.len(),
        ...
        decreases keys.len() - i,
    {
        ...
        let replacement = crate::k2_term::push_comp(
            &mut arena,
            vstd::slice::slice_to_vec(name),
            args,
        );
        proof {
    ...
    proof {
        partial_done(model, fs, base as nat);
    }
    (arena, current, next)
}
...
pub fn number_exec(arena: &mut ETermArena, root: usize, base: usize) -> (out: (usize, usize))
    ...
    ensures
        ...
        root_ok(final(arena), out.0),
        out.1 <= final(arena).nodes.len(),
        (final(arena)@[out.0 as int], out.1 as nat) == number(
