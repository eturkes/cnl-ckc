...
pub fn v1_aggregate_check_impl(mpath: &[u8], m: &ESrc, pls: &Vec<ESrc>, pys: &Vec<ESrc>) -> (r: EOut)
    requires
        ...
        r@ == agg_output(mpath@, m@, srcs(pls@), srcs(pys@)),
{
    ...
    if let Some(out) = crate::k2_aggregate::coverage_check_exec(&mut arena, &loaded.db, &pairs, Ghost(models)) {
        return out;
    }
    proof {
    ...
    if let Some(index) = crate::k2_aggregate::first_unproved_exec(&mut arena, &loaded.db, &obs) {
        ...
        proof {
            reveal_byteslit(b"obligation_failed"); reveal_strlit("obligation_failed");
...
pub fn v1_recursion_check_impl(mpath: &[u8], m: &ESrc, pls: &Vec<ESrc>, pys: &Vec<ESrc>) -> (r: EOut)
    requires
        cells_ok(m@, srcs(pls@), srcs(pys@)),
    ensures
        r@ == recursion_output(mpath@, m@, srcs(pls@), srcs(pys@)),
{
    ...
    if rows.len() == 0 { return crate::k2_output::meter_out(0, 0, true); }
    let mut arena = crate::k2_reject::empty_arena();
    let loaded = match crate::k2_load::load_stage_exec(&mut arena, &rows, pls) {
        Err(out) => return out,
        Ok(loaded) => loaded,
    ...
    if let Some(index) = crate::k2_recursion::first_recursive(&mut arena, &loaded.db, &indices) {
        proof { assert(crate::k2_engine::clause_valid(arena.nodes@, &loaded.db@[index as int])); }
        if let Some(out) = crate::k2_recursion::left_recursive_error(&mut arena, &loaded.db[index]) {
            return out;
        }
    }
...
pub fn v1_answer_impl(mpath: &[u8], m: &ESrc, pls: &Vec<ESrc>, pys: &Vec<ESrc>, query: &ESrc, qsha: &[u8]) -> (r: EOut)
    requires
        cells_ok(m@, srcs(pls@), srcs(pys@)),
    ensures
        r@ == answer_output(mpath@, m@, srcs(pls@), srcs(pys@), query@, qsha@),
{
    ...
    proof { crate::k2_answers::query_prefix(before_composition, arena.nodes@, &query); }
