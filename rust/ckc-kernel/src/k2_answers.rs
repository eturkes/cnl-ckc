...
use crate::k2_output::{atom_root, int_root, comp1, comp2};
...
#[cfg(verus_keep_ghost)]
use crate::k2_term::{arena_ok, root_ok, node_ok};
#[cfg(verus_keep_ghost)]
...
fn semantic_name(name: &Vec<u8>, arity: usize) -> (out: bool)
    ensures out == ckc_spec::v1text::is_semantic_pred(name@, arity as nat),
{
    let mut i = 2usize;
    while i < 9
        invariant 2 <= i <= 9,
            forall|j: int| 2 <= j < i ==> #[trigger] ckc_spec::v1text::indicator(j) != (name@, arity as nat),
        decreases 9 - i,
    {
        let (candidate, count) = crate::k2_recursion::indicator_exec(i);
        if count == arity && crate::k2_engine::vec_equal(&candidate, name) {
            proof { assert(ckc_spec::v1text::indicator(i as int) == (name@, arity as nat)); }
            return true;
        }
        i += 1;
    }
...
spec fn walk_goals(goals: Seq<Term>) -> Option<Term>
    decreases goals.len(),
{
    if goals.len() == 0 { None } else {
        match goal_walk(goals[0]) { Some(why) => Some(why), None => walk_goals(goals.drop_first()) }
    }
}

proof fn walk_goals_concat(left: Seq<Term>, right: Seq<Term>)
    ensures walk_goals(left + right) == match walk_goals(left) { Some(why) => Some(why), None => walk_goals(right) },
    decreases left.len(),
{
    ...
    } else { assert_seqs_equal!(left + right == right); }
}

spec fn comma_goal(t: Term) -> bool {
    match t { Term::Comp(name, args) => name == ckc_spec::engine::comma_name() && args.len() == 2, _ => false }
}

fn goal_offender(arena: &ETermArena, goal: usize) -> (out: Option<usize>)
    requires root_ok(arena, goal),
    ensures out.is_none() == goal_walk(arena@[goal as int]).is_none(),
        out matches Some(root) ==> root < arena.nodes.len() && !comma_goal(arena@[root as int])
            && goal_walk(arena@[root as int]).is_some()
            && goal_walk(arena@[root as int]) == goal_walk(arena@[goal as int]),
{
    hide(goal_walk);
    hide(walk_goals);
    ...
    while pending.len() > 0
        invariant root_ok(arena, goal), roots_valid(arena.nodes@, pending@),
        ...
        match &arena.nodes[root].kind {
            ENodeKind::Comp { name, child_roots, .. } => {
                ...
                if args.len() == 2 && crate::k2_engine::vec_equal(name, &comma) {
                    ...
                    pending = next;
                    proof {
                        assert forall|j: int| 0 <= j < pending.len() implies pending@[j] < arena.nodes.len() by {
                            if j < children.len() { assert(pending@[j] == children[j]); }
                            else { assert(pending@[j] == rest[j - children.len()]); }
                        }
                ...
                } else if !semantic_name(name, args.len()) { return Some(root); }
            },
            _ => return Some(root),
        }
    }
...
pub fn query_error(arena: &mut ETermArena, why: usize) -> (out: EOut)
    requires root_ok(old(arena), why),
        ...
        out@ == query_reject(old(arena)@[why as int]),
{
    ...
    proof {
        reveal_byteslit(b"query_file"); reveal_strlit("query_file"); reveal(ckc_spec::v1text::ascii);
...
pub fn query_atom_error(why: &[u8]) -> (out: EOut)
    ensures out@ == query_reject(Term::Atom(why@)),
{
...
fn goal_leaf_error(arena: &mut ETermArena, root: usize) -> (out: EOut)
    requires root_ok(old(arena), root), !comma_goal(old(arena)@[root as int]),
        goal_walk(old(arena)@[root as int]).is_some(),
    ensures arena_ok(final(arena)), old(arena).nodes@.is_prefix_of(final(arena).nodes@),
        Some(out@) == (match goal_walk(old(arena)@[root as int]) { Some(why) => Some(query_reject(why)), None => None }),
{
    proof { assert(node_ok(arena.nodes@, root as int)); }
    let variable: &[u8] = b"goal_variable";
    let foreign: &[u8] = b"goal_foreign";
    proof {
        reveal_byteslit(b"goal_variable"); reveal_strlit("goal_variable");
        reveal_byteslit(b"goal_foreign"); reveal_strlit("goal_foreign"); reveal(ckc_spec::v1text::ascii);
...
pub fn goal_check_exec(arena: &mut ETermArena, goal: usize) -> (out: Option<EOut>)
    requires root_ok(old(arena), goal),
    ensures arena_ok(final(arena)), old(arena).nodes@.is_prefix_of(final(arena).nodes@),
        crate::k2_output::option_out_view(out)
            == (match goal_walk(old(arena)@[goal as int]) { Some(why) => Some(query_reject(why)), None => None }),
{
    match goal_offender(arena, goal) { Some(root) => Some(goal_leaf_error(arena, root)), None => None }
}

pub fn variable_key(arena: &ETermArena, root: usize) -> (out: Option<usize>)
    requires root_ok(arena, root),
    ensures out.is_some() == (arena@[root as int] is Var),
        out matches Some(key) ==> arena@[root as int] == Term::Var(key as nat),
{
    proof { assert(node_ok(arena.nodes@, root as int)); }
    match &arena.nodes[root].kind { ENodeKind::Var { key, .. } => Some(*key), _ => None }
}

fn atom_is(arena: &ETermArena, root: usize, bytes: &Vec<u8>) -> (out: bool)
    requires root_ok(arena, root),
    ensures out == (arena@[root as int] == Term::Atom(bytes@)),
{
    proof { assert(node_ok(arena.nodes@, root as int)); }
    match &arena.nodes[root].kind {
...
fn is_atom(arena: &ETermArena, root: usize) -> (out: bool)
    requires root_ok(arena, root),
    ensures out == (arena@[root as int] is Atom),
{
    proof { assert(node_ok(arena.nodes@, root as int)); }
...
fn descriptor_ok(arena: &ETermArena, root: usize) -> (out: bool)
    requires root_ok(arena, root),
    ensures out == ckc_spec::answers::desc_ok(arena@[root as int]),
{
    ...
    proof {
        reveal_byteslit(b"noun"); reveal_strlit("noun");
        reveal_byteslit(b"wh"); reveal_strlit("wh");
        reveal_byteslit(b"who"); reveal_strlit("who");
        reveal_byteslit(b"what"); reveal_strlit("what"); reveal(ckc_spec::v1text::ascii);
...
fn key_seen(seen: &Vec<usize>, key: usize) -> (out: bool)
    ensures out == seen@.contains(key),
{
    let mut i = 0usize;
    while i < seen.len()
        invariant i <= seen.len(), forall|j: int| 0 <= j < i ==> seen@[j] != key,
        decreases seen.len() - i,
    {
        if seen[i] == key { proof { assert(seen@[i as int] == key); } return true; }
        i += 1;
    }
    false
}
...
fn answer_var_error(i: usize, why: &[u8]) -> (out: EOut)
    ensures out@ == query_reject(Term::Comp(ckc_spec::v1text::ascii("answer_var"@),
        seq![Term::Int(i as int), Term::Atom(why@)])),
{
    ...
    proof {
        reveal_byteslit(b"answer_var"); reveal_strlit("answer_var"); reveal(ckc_spec::v1text::ascii);
...
fn answer_row_error(i: usize, name: &[u8]) -> (out: EOut)
    ensures out@ == query_reject(Term::Comp(name@, seq![Term::Int(i as int)])),
{
    let mut arena = crate::k2_reject::empty_arena();
    let index = int_root(&mut arena, i);
...
pub fn rows_check_exec(arena: &ETermArena, rows: &Vec<usize>, goal: usize) -> (out: Option<EOut>)
    requires root_ok(arena, goal), roots_valid(arena.nodes@, rows@),
    ensures crate::k2_output::option_out_view(out) ==
        match ckc_spec::answers::rows_check(root_terms(arena.nodes@, rows@), 1, arena@[goal as int], Seq::empty()) {
            Some(why) => Some(query_reject(why)), None => None,
        },
{
    ...
    let mut i = 0usize;
    proof {
        reveal_byteslit(b"answer"); reveal_strlit("answer");
        reveal_byteslit(b"answer_shape"); reveal_strlit("answer_shape");
        reveal_byteslit(b"nonvar"); reveal_strlit("nonvar");
        reveal_byteslit(b"absent"); reveal_strlit("absent");
        reveal_byteslit(b"duplicate"); reveal_strlit("duplicate");
        reveal_byteslit(b"answer_desc"); reveal_strlit("answer_desc"); reveal(ckc_spec::v1text::ascii);
    ...
    while i < rows.len()
        invariant root_ok(arena, goal), roots_valid(arena.nodes@, rows@), i <= rows.len(),
            models == root_terms(arena.nodes@, rows@),
            ckc_spec::answers::rows_check(models, 1, arena@[goal as int], Seq::empty())
                == ckc_spec::answers::rows_check(models.skip(i as int), i as nat + 1, arena@[goal as int],
                    seen@.map_values(|x: usize| x as nat)),
            answer@ == ckc_spec::v1text::ascii("answer"@), shape@ == ckc_spec::v1text::ascii("answer_shape"@),
            nonvar@ == ckc_spec::v1text::ascii("nonvar"@), absent@ == ckc_spec::v1text::ascii("absent"@),
            duplicate@ == ckc_spec::v1text::ascii("duplicate"@), desc@ == ckc_spec::v1text::ascii("answer_desc"@),
        decreases rows.len() - i,
    {
        ...
        if !crate::k2_engine::occurs_root(arena, key, goal) { return Some(answer_var_error(i + 1, absent)); }
        proof {
        ...
        if key_seen(&seen, key) { return Some(answer_var_error(i + 1, duplicate)); }
        if !descriptor_ok(arena, args[1]) { return Some(answer_row_error(i + 1, desc)); }
...
proof fn clauses_byte_bound(clauses: Seq<ckc_spec::v1text::DocClause>)
    ensures clauses.len() <= ckc_spec::v1text::clauses_bytes(clauses).len(),
...
proof fn bundles_census(bundles: Seq<ckc_spec::v1text::Bundle>)
    ensures
        ckc_spec::answers::clause_count(bundles)
            == bundles.map_values(|b: ckc_spec::v1text::Bundle| b.clauses).flatten().len(),
...
fn parsed_term_count(arena: &ETermArena, bytes: &[u8], parsed: &crate::v1_term_impl::EParsedV1) -> (out: usize)
    ...
    ensures out as nat == ckc_spec::answers::term_count(parsed@),
{
    match parsed.class {
        crate::v1_term_impl::EV1Class::Doc => {
            let ghost doc = choose|d: ckc_spec::v1text::DocFile| parsed@ == ckc_spec::v1text::V1File::Doc(d);
            proof {
                ...
                crate::v1_term_impl::doc_clauses_roots_elim(arena.nodes@, parsed.clauses@,
...
pub proof fn query_prefix(before: Seq<ENode>, after: Seq<ENode>, query: &EQuery)
    requires before.is_prefix_of(after), query_ok(before, query),
    ensures query_ok(after, query), root_terms(after, query.rows@) == root_terms(before, query.rows@),
{
    crate::k2_engine::roots_models_prefix(before, after, query.rows@);
    assert(after[query.goal as int] == before[query.goal as int]);
}

pub open spec fn custody_view(nodes: Seq<ENode>, result: Result<EQuery, EOut>)
    -> Result<(QueryFile, Seq<Term>), ckc_spec::replay::Out>
{
    match result { Ok(query) => Ok((query.file@, root_terms(nodes, query.rows@))), Err(out) => Err(out@) }
}
        ...
        custody_view(final(arena).nodes@, out) == ckc_spec::answers::custody(source@),
{
    ...
    let bytes = match source {
        ESrc::Missing => {
            proof { reveal(ckc_spec::answers::custody); }
    ...
    if !matches!(parsed.class, crate::v1_term_impl::EV1Class::Query) {
        ...
        proof {
            reveal(ckc_spec::answers::custody);
            reveal_byteslit(b"term_count"); reveal_strlit("term_count");
            reveal(ckc_spec::v1text::ascii);
    ...
    if let Some(out) = failed { return Err(out); }
    let rows = match crate::k2_walk::list_items_exec(arena, answers) {
        None => {
            let name: &[u8] = b"answers_list";
            proof {
                reveal_byteslit(b"answers_list"); reveal_strlit("answers_list");
                reveal(ckc_spec::v1text::ascii);
    ...
    if let Some(out) = rows_check_exec(arena, &rows, goal) { return Err(out); }
...
fn project_roots_inner(mut arena: ETermArena, rows: &Vec<usize>) -> (out: (Vec<usize>, ETermArena))
    requires arena_ok(&arena), roots_valid(arena.nodes@, rows@),
    ensures arena_ok(&out.1), arena.nodes@.is_prefix_of(out.1.nodes@), roots_valid(out.1.nodes@, out.0@),
        root_terms(out.1.nodes@, out.0@)
            == root_terms(arena.nodes@, rows@).map_values(|r: Term| ckc_spec::replay::arg(r, 0)),
{
    ...
    let mut i = 0usize;
    while i < rows.len()
        invariant arena_ok(&arena), base.is_prefix_of(arena.nodes@),
            roots_valid(base, rows@), roots_valid(arena.nodes@, rows@),
            models == root_terms(base, rows@), models == root_terms(arena.nodes@, rows@),
            i <= rows.len(), result.len() == i, roots_valid(arena.nodes@, result@),
            root_terms(arena.nodes@, result@) == models.take(i as int).map_values(|r: Term| ckc_spec::replay::arg(r, 0)),
        decreases rows.len() - i,
    {
    ...
    proof { assert_seqs_equal!(models.take(i as int) == models); }
    (result, arena)
}

fn project_roots(arena: &mut ETermArena, rows: &Vec<usize>) -> (out: Vec<usize>)
    requires arena_ok(old(arena)), roots_valid(old(arena).nodes@, rows@),
    ensures arena_ok(final(arena)), old(arena).nodes@.is_prefix_of(final(arena).nodes@),
        roots_valid(final(arena).nodes@, out@), root_terms(final(arena).nodes@, out@)
            == root_terms(old(arena).nodes@, rows@).map_values(|r: Term| ckc_spec::replay::arg(r, 0)),
{
...
proof fn rows_prefix(before: Seq<ENode>, after: Seq<ENode>, rows: Seq<Vec<usize>>)
    requires before.is_prefix_of(after), crate::k2_engine::rows_valid(before, rows),
    ensures crate::k2_engine::rows_valid(after, rows),
        crate::k2_engine::rows_view(after, rows) == crate::k2_engine::rows_view(before, rows),
{
    assert forall|i: int| 0 <= i < rows.len() implies
        #[trigger] roots_valid(after, rows[i]@)
        && root_terms(after, rows[i]@) == root_terms(before, rows[i]@) by {
        crate::k2_engine::roots_models_prefix(before, after, rows[i]@);
    }
    assert forall|i: int| 0 <= i < rows.len() implies
        #[trigger] crate::k2_engine::rows_view(after, rows)[i] == crate::k2_engine::rows_view(before, rows)[i] by {
        crate::k2_engine::roots_models_prefix(before, after, rows[i]@);
    }
...
fn row_ground_exec(arena: &ETermArena, row: &Vec<usize>) -> (out: bool)
    requires arena_ok(arena), roots_valid(arena.nodes@, row@),
    ensures out == ckc_spec::answers::row_ground(root_terms(arena.nodes@, row@)),
{
    let mut i = 0usize;
    while i < row.len()
        invariant arena_ok(arena), roots_valid(arena.nodes@, row@), i <= row.len(),
            forall|j: int| 0 <= j < i ==> ckc_spec::term::ground(#[trigger] root_terms(arena.nodes@, row@)[j]),
        decreases row.len() - i,
    {
        if !crate::k2_walk::ground_root(arena, row[i]) {
            proof { assert(!ckc_spec::term::ground(root_terms(arena.nodes@, row@)[i as int])); }
            return false;
        }
        i += 1;
    }
...
fn some_nonground_exec(arena: &ETermArena, rows: &Vec<Vec<usize>>) -> (out: bool)
    requires arena_ok(arena), crate::k2_engine::rows_valid(arena.nodes@, rows@),
    ensures out == ckc_spec::answers::some_nonground(crate::k2_engine::rows_view(arena.nodes@, rows@)),
{
    let mut i = 0usize;
    while i < rows.len()
        invariant arena_ok(arena), crate::k2_engine::rows_valid(arena.nodes@, rows@), i <= rows.len(),
            forall|j: int| 0 <= j < i ==> ckc_spec::answers::row_ground(#[trigger] crate::k2_engine::rows_view(arena.nodes@, rows@)[j]),
        decreases rows.len() - i,
    {
        if !row_ground_exec(arena, &rows[i]) {
            proof { assert(!ckc_spec::answers::row_ground(crate::k2_engine::rows_view(arena.nodes@, rows@)[i as int])); }
            return true;
        }
        i += 1;
    }
...
proof fn list_ground(terms: Seq<Term>)
    requires ckc_spec::answers::row_ground(terms),
    ensures ckc_spec::term::ground(ckc_spec::engine::list_term(terms)),
    decreases terms.len(),
{
    reveal_with_fuel(ckc_spec::engine::list_term, 1);
    if terms.len() > 0 {
        assert forall|i: int| 0 <= i < terms.drop_first().len() implies
            ckc_spec::term::ground(#[trigger] terms.drop_first()[i]) by {
...
fn sol_terms_inner(mut arena: ETermArena, rows: &Vec<Vec<usize>>) -> (out: (Vec<usize>, ETermArena))
    ...
    ensures arena_ok(&out.1), arena.nodes@.is_prefix_of(out.1.nodes@), roots_valid(out.1.nodes@, out.0@),
        root_terms(out.1.nodes@, out.0@)
            == crate::k2_engine::rows_view(arena.nodes@, rows@).map_values(|r: Seq<Term>| ckc_spec::answers::sol_term(r)),
        ckc_spec::answers::row_ground(root_terms(out.1.nodes@, out.0@)),
{
    ...
    while i < rows.len()
        invariant arena_ok(&arena), base.is_prefix_of(arena.nodes@),
            crate::k2_engine::rows_valid(base, rows@), crate::k2_engine::rows_valid(arena.nodes@, rows@),
            models == crate::k2_engine::rows_view(base, rows@), models == crate::k2_engine::rows_view(arena.nodes@, rows@),
            !ckc_spec::answers::some_nonground(models), i <= rows.len(), result.len() == i,
            roots_valid(arena.nodes@, result@), ckc_spec::answers::row_ground(root_terms(arena.nodes@, result@)),
            root_terms(arena.nodes@, result@) == models.take(i as int).map_values(|r: Seq<Term>| ckc_spec::answers::sol_term(r)),
        ...
        decreases rows.len() - i,
    {
        ...
        proof {
            ...
            assert forall|j: int| 0 <= j < result.len() implies
                ckc_spec::term::ground(#[trigger] root_terms(arena.nodes@, result@)[j]) by {
                if j < previous.len() { assert(result@[j] == previous[j]); }
                else { assert(result@[j] == root); }
            }
        }
        i += 1;
    }
...
fn solutions_root(arena: &mut ETermArena, rows: &Vec<Vec<usize>>) -> (out: usize)
    requires arena_ok(old(arena)), crate::k2_engine::rows_valid(old(arena).nodes@, rows@),
    ...
    ensures arena_ok(final(arena)), old(arena).nodes@.is_prefix_of(final(arena).nodes@), root_ok(final(arena), out),
        final(arena)@[out as int] == Term::Comp(ckc_spec::v1text::ascii("solutions"@), seq![ckc_spec::engine::list_term(
            ckc_spec::engine::sort_unique(crate::k2_engine::rows_view(old(arena).nodes@, rows@)
                .map_values(|r: Seq<Term>| ckc_spec::answers::sol_term(r))))]),
{
    ...
    proof {
        ...
        assert forall|i: int| 0 <= i < terms.len() implies
            ckc_spec::term::ground(arena@[#[trigger] terms@[i] as int]) by {
            assert(ckc_spec::term::ground(root_terms(arena.nodes@, terms@)[i]));
        }
    }
    let sorted = crate::k2_sort::sort_unique(arena, &terms);
    proof { crate::k2_aggregate::sort_views(arena, sorted@); }
        ...
        final(arena)@[out as int] == ckc_spec::answers::limit(),
{
    ...
    proof {
        reveal_byteslit(b"limit"); reveal_strlit("limit");
        reveal_byteslit(b"indeterminate"); reveal_strlit("indeterminate"); reveal(ckc_spec::v1text::ascii);
...
pub open spec fn result_view(nodes: Seq<ENode>, result: Result<usize, EOut>) -> Result<Term, ckc_spec::replay::Out> {
    match result { Ok(root) => Ok(nodes[root as int].term@), Err(out) => Err(out@) }
}

pub fn answer_result_exec(arena: &mut ETermArena, db: &Vec<crate::k2_engine::EClause>, query: &EQuery)
    -> (out: Result<usize, EOut>)
    requires arena_ok(old(arena)), crate::k2_engine::db_valid(old(arena).nodes@, db@), query_ok(old(arena).nodes@, query),
    ensures arena_ok(final(arena)), old(arena).nodes@.is_prefix_of(final(arena).nodes@),
        out matches Ok(root) ==> root_ok(final(arena), root),
        result_view(final(arena).nodes@, out) == ckc_spec::answers::answer_result(
            crate::k2_engine::db_view(old(arena).nodes@, db@), query.file@, root_terms(old(arena).nodes@, query.rows@)),
{
    ...
    let result = crate::k2_engine::solve(arena, db, query.goal, 100, sol, query.rows.len() > 0, 100000);
    proof {
    ...
    match result {
        crate::k2_engine::EROut::Sol => {
            let name: &[u8] = b"yes";
            proof {
                reveal_byteslit(b"yes"); reveal_strlit("yes"); reveal(ckc_spec::v1text::ascii);
        ...
        crate::k2_engine::EROut::End { complete, rows } => {
            if query.rows.len() == 0 {
                if !complete { return Ok(limit_root(arena)); }
                let name: &[u8] = b"no";
                let why: &[u8] = b"finite_failure";
                proof {
                    reveal_byteslit(b"no"); reveal_strlit("no");
                    reveal_byteslit(b"finite_failure"); reveal_strlit("finite_failure"); reveal(ckc_spec::v1text::ascii);
            ...
            if some_nonground_exec(arena, &rows) {
                ...
                proof {
                    reveal_byteslit(b"nonground_solution"); reveal_strlit("nonground_solution");
                    reveal(ckc_spec::v1text::ascii);
...
pub fn print_answers_exec(arena: &mut ETermArena, qid: &[u8], qsha: &[u8], result: usize) -> (out: EOut)
    requires root_ok(old(arena), result),
    ensures arena_ok(final(arena)), old(arena).nodes@.is_prefix_of(final(arena).nodes@),
        out@ == ckc_spec::replay::ok(ckc_spec::v1text::print_answers(ckc_spec::v1text::AnswersFile {
            qid: qid@, qsha: qsha@, result: old(arena)@[result as int],
        })),
{
    let ghost before = arena.nodes@;
    let version: &[u8] = b"v1";
    ...
    let prefix: &[u8] = b"% ";
    let suffix: &[u8] = b" answered against the loaded composition by ace_to_pl answer mode; do not edit.\n";
    proof {
        ...
        reveal_byteslit(b"$guideline_answers"); reveal_strlit("$guideline_answers");
        reveal_byteslit(b"% "); reveal_strlit("% ");
        reveal_byteslit(b" answered against the loaded composition by ace_to_pl answer mode; do not edit.\n");
        ...
        assert(suffix@ == ckc_spec::v1text::ascii(" answered against the loaded composition by ace_to_pl answer mode; do not edit.\n"@));
    }
    ...
    proof {
        ...
        crate::k2_term::child_terms_match(arena.nodes@, children@, seq![
            Term::Atom(version@), Term::Atom(qid@), Term::Comp(query_name@, seq![Term::Atom(qsha@)]),
