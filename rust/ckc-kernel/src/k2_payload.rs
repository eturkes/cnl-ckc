...
pub proof fn ob_prefix(before: Seq<ENode>, after: Seq<ENode>, ob: &EOb)
    ...
    ensures
        ob_valid(after, ob),
        ob_view(before, ob) == ob_view(after, ob),
{
...
pub proof fn obs_prefix(before: Seq<ENode>, after: Seq<ENode>, obs: Seq<EOb>)
    requires
        before.is_prefix_of(after),
        obs_valid(before, obs),
    ensures
        obs_valid(after, obs),
        obs_view(before, obs) == obs_view(after, obs),
{
    assert forall|i: int| 0 <= i < obs.len() implies {
        &&& ob_valid(after, &obs[i])
        &&& ob_view(before, &obs[i]) == ob_view(after, &obs[i])
    } by {
        ob_prefix(before, after, &obs[i]);
    }
    assert_seqs_equal!(obs_view(before, obs) == obs_view(after, obs));
}

fn extract_ob(arena: &ETermArena, root: usize, Ghost(expected): Ghost<Option<Ob>>) -> (out: Option<
    EOb,
>)
    requires
        root_ok(arena, root),
    ...
    ensures
        out matches Some(ob) ==> ob_valid(arena.nodes@, &ob) && ob_term(ob_view(arena.nodes@, &ob))
            == arena@[root as int],
        expected matches Some(model) ==> out matches Some(ob) && ob_view(arena.nodes@, &ob)
            == model,
{
    ...
    if !literal_matches(arena, root, &proof_name, 5) {
        return None;
    }
    ...
    if !literal_matches(arena, variant_root, &variant_name, 1) || !literal_matches(
        arena,
        witness_root,
        &witness_name,
        1,
    ) || !literal_matches(arena, prove_root, &prove_name, 1) {
        return None;
    }
    ...
    proof {
        if let Some(model) = expected {
            ...
            assert(arena@[witness_root as int] == Term::Comp(
                witness_name@,
                seq![ckc_spec::engine::list_term(model.facts)],
            ));
            assert(arena@[prove_root as int] == Term::Comp(
                prove_name@,
                seq![ckc_spec::engine::list_term(model.heads)],
            ));
            assert(root_terms(arena.nodes@, variant_args@) == seq![model.k]);
            assert(root_terms(arena.nodes@, witness_args@) == seq![
                ckc_spec::engine::list_term(model.facts),
            ]);
            assert(root_terms(arena.nodes@, prove_args@) == seq![
                ckc_spec::engine::list_term(model.heads),
            ]);
    ...
    let facts = match list_items_exec(arena, facts_root) {
        Some(roots) => roots,
        None => return None,
    };
    let heads = match list_items_exec(arena, heads_root) {
        Some(roots) => roots,
        None => return None,
    };
    proof {
...
proof fn ground_keys_fit_all(terms: Seq<Term>)
    ...
    ensures
        crate::v1_term_impl::terms_keys_fit(terms),
...
pub proof fn obs_wf_at(obs: Seq<Ob>, i: int)
    requires
        ...
        0 <= i < obs.len(),
    ensures
        ckc_spec::term::wf_term(ob_term(obs[i])),
...
proof fn term_line_suffix(term: Term)
    ensures
        ckc_spec::v1text::term_line(term) == ckc_spec::v1text::term_bytes(term) + seq![
            0x2eu8,
            0x0au8,
        ],
{
...
proof fn ob_term_guide(bytes: Seq<u8>, start: int, ob: Ob)
    requires
        ob_at(bytes, start, ob),
    ensures
        crate::v1_term_impl::term_at(
            bytes,
            start,
            start + ckc_spec::v1text::term_bytes(ob_term(ob)).len(),
            ob_term(ob),
        ),
...
fn parse_ob_line(
    ...
    bytes: &[u8],
    start: usize,
    Ghost(expected): Ghost<Option<Ob>>,
    ...
    ensures
        ...
        out.0 matches Some((ob, next)) ==> {
            ...
            &&& bytes@.subrange(start as int, next as int) == ckc_spec::v1text::term_line(
                ob_term(ob_view(out.1.nodes@, &ob)),
            )
        },
        expected matches Some(model) ==> out.0 matches Some((ob, next)) && ob_view(
            out.1.nodes@,
            &ob,
        ) == model && next == start + ckc_spec::v1text::term_line(ob_term(model)).len(),
{
    let mut arena = input_arena;
    let ghost guided = match expected {
        Some(ob) => Some(
            crate::v1_term_impl::GTermExpected {
                term: ob_term(ob),
    ...
    proof {
        if let Some(ob) = expected {
            ob_term_guide(bytes@, start as int, ob);
        }
    }
    let mut tracker = crate::v1_term_impl::EVarTracker {
        next: 0,
        stream: Ghost(Seq::empty()),
        valid: true,
    };
    proof {
    ...
    let mut at = start;
    let parsed = match crate::v1_term_impl::parse_term(
        bytes,
        start,
        &mut arena,
        Ghost(guided),
    ...
    if !parsed.parsed.ground || !parsed.parsed.no_dollar {
        return (None, arena);
    }
    if parsed.end >= bytes.len() || bytes[parsed.end] != 0x2e {
        return (None, arena);
    }
    if parsed.end + 1 >= bytes.len() || bytes[parsed.end + 1] != 0x0a {
        return (None, arena);
    }
...
pub proof fn payload_print_concat(left: Seq<Ob>, right: Seq<Ob>)
    ensures
        ckc_spec::replay::print_payload(left + right) == ckc_spec::replay::print_payload(left)
            + ckc_spec::replay::print_payload(right),
{
...
proof fn payload_print_step(obs: Seq<Ob>)
    requires
        obs.len() > 0,
    ensures
        ckc_spec::replay::print_payload(obs) == ckc_spec::v1text::term_line(ob_term(obs[0]))
...
pub proof fn payload_print_push(obs: Seq<Ob>, ob: Ob)
    ensures
        ckc_spec::replay::print_payload(obs.push(ob)) == ckc_spec::replay::print_payload(obs)
            + ckc_spec::v1text::term_line(ob_term(ob)),
{
...
proof fn payload_print_nonempty(obs: Seq<Ob>)
    requires
        obs.len() > 0,
    ensures
        ckc_spec::replay::print_payload(obs).len() > 0,
{
...
proof fn selected_payload(bytes: Seq<u8>)
    requires
        ckc_spec::replay::payload_accepts(bytes),
    ensures
        wf_obs(ckc_spec::replay::the_payload(bytes)),
...
fn parse_payload_inner(input_arena: ETermArena, bytes: &[u8]) -> (out: (
    Option<Vec<EOb>>,
    ETermArena,
))
    requires
    ...
    ensures
        ...
        out.0 matches Some(obs) ==> obs_valid(out.1.nodes@, obs@) && wf_obs(
            obs_view(out.1.nodes@, obs@),
        ) && obs_view(out.1.nodes@, obs@) == ckc_spec::replay::the_payload(bytes@),
{
    ...
    while pos < bytes.len()
        invariant
            ...
            wf_obs(obs_view(arena.nodes@, out@)),
            bytes@.take(pos as int) == ckc_spec::replay::print_payload(
                obs_view(arena.nodes@, out@),
            ),
            canonical == ckc_spec::replay::payload_accepts(bytes@),
            models == ckc_spec::replay::the_payload(bytes@),
            canonical ==> wf_obs(models) && out.len() <= models.len() && obs_view(
                arena.nodes@,
                out@,
            ) == models.take(out.len() as int) && bytes@.skip(pos as int)
                == ckc_spec::replay::print_payload(models.skip(out.len() as int)),
        decreases bytes.len() - pos,
    {
        ...
        let ghost expected = if canonical {
            Some(models[out.len() as int])
        } else {
            None
        };
        proof {
            if canonical {
                ...
                assert forall|i: int| 0 <= i < rest.len() implies ckc_spec::term::wf_term(
                    ob_term(rest[i]),
                ) && ckc_spec::term::ground(ob_term(rest[i])) && ckc_spec::term::no_dollar_var(
                    ob_term(rest[i]),
                ) by {
        ...
        proof {
            ...
            assert forall|i: int| 0 <= i < out@.len() implies ckc_spec::term::wf_term(
                ob_term(obs_view(arena.nodes@, out@)[i]),
            ) && ckc_spec::term::ground(ob_term(obs_view(arena.nodes@, out@)[i]))
                && ckc_spec::term::no_dollar_var(ob_term(obs_view(arena.nodes@, out@)[i])) by {
                if i < before_out.len() {
                    obs_wf_at(before_models, i);
                } else {
                    obs_wf_at(seq![model], 0);
                }
            }
    ...
    proof {
        ...
        reveal(ckc_spec::replay::payload_accepts);
        assert(exists|obs: Seq<Ob>| #[trigger]
            wf_obs(obs) && ckc_spec::replay::print_payload(obs) == bytes@) by {
            assert(wf_obs(obs_view(arena.nodes@, out@)));
        }
        assert(canonical);
        let rest = models.skip(out.len() as int);
        if rest.len() > 0 {
            payload_print_nonempty(rest);
            assert(false);
        }
...
pub fn parse_payload_file(arena: &mut ETermArena, bytes: &[u8]) -> (out: Option<Vec<EOb>>)
    ...
    ensures
        ...
        out matches Some(obs) ==> obs_valid(final(arena).nodes@, obs@) && wf_obs(
            obs_view(final(arena).nodes@, obs@),
...
pub open spec fn selected_ob(nodes: Seq<ENode>, obs: Seq<EOb>, index: Option<usize>) -> Option<Ob> {
    match index {
        Some(i) => Some(ob_view(nodes, &obs[i as int])),
        None => None,
    }
}
...
fn first_empty_index(arena: &ETermArena, obs: &Vec<EOb>) -> (out: Option<usize>)
    ...
    ensures
        ...
        selected_ob(arena.nodes@, obs@, out) == ckc_spec::replay::first_empty(
            obs_view(arena.nodes@, obs@),
        ),
{
    ...
    while i < obs.len()
        invariant
            ...
            ckc_spec::replay::first_empty(models) == ckc_spec::replay::first_empty(
    ...
    ensures
        ...
        out@ == ckc_spec::replay::proof_fail(
            Term::Comp(
                name@,
                seq![
                    ob_view(old(arena).nodes@, ob).docid,
                    ob_view(old(arena).nodes@, ob).s,
...
pub open spec fn payload_stage_view(
    nodes: Seq<ENode>,
...
fn payload_stage_inner(input_arena: ETermArena, rows: &Vec<ERow>, pys: &Vec<ESrc>) -> (out: (
    Result<Vec<EOb>, EOut>,
    ETermArena,
))
    requires
    ...
    ensures
        arena_ok(&out.1),
        input_arena.nodes@.is_prefix_of(out.1.nodes@),
        out.0 matches Ok(obs) ==> obs_valid(out.1.nodes@, obs@) && wf_obs(
            obs_view(out.1.nodes@, obs@),
        ),
        ...
        ),
{
    ...
    proof {
        reveal_byteslit(b"payload_term");
        reveal_strlit("payload_term");
        reveal_byteslit(b"empty_obligation");
        reveal_strlit("empty_obligation");
        reveal(ckc_spec::v1text::ascii);
    ...
    while i < rows.len()
        invariant
            ...
            cells == ckc_spec::replay::srcs(pys@),
            obs_valid(arena.nodes@, out@),
            wf_obs(obs_view(arena.nodes@, out@)),
            ckc_spec::replay::payload_stage(models, cells, 0, Seq::empty())
                == ckc_spec::replay::payload_stage(
                models,
                cells,
                i as nat,
                obs_view(arena.nodes@, out@),
        ...
        decreases rows.len() - i,
    {
        ...
        let ghost before_models = obs_view(before_nodes, before_out);
        proof {
            reveal_with_fuel(ckc_spec::replay::payload_stage, 1);
        }
        ...
        proof {
            assert(raw@ == ckc_spec::replay::src_bytes(cells[i as int]));
        }
        let mut parsed = match parse_payload_file(&mut arena, raw) {
            Some(obs) => obs,
            None => return (
                Err(crate::k2_output::named_atom_error(payload_name, &rows[i].payload, false)),
                arena,
            ),
        };
        proof {
        ...
        if let Some(index) = first_empty_index(&arena, &parsed) {
            proof {
                assert(ob_valid(arena.nodes@, &parsed@[index as int]));
            }
        ...
        proof {
            assert forall|j: int| 0 <= j < out@.len() implies ob_valid(arena.nodes@, &out@[j]) by {
                if j < before_out.len() {
                    assert(out@[j] == before_out[j]);
                } else {
                    assert(out@[j] == right[j - before_out.len()]);
                }
            }
            ...
            assert forall|j: int| 0 <= j < out@.len() implies ckc_spec::term::wf_term(
                ob_term(obs_view(arena.nodes@, out@)[j]),
            ) && ckc_spec::term::ground(ob_term(obs_view(arena.nodes@, out@)[j]))
                && ckc_spec::term::no_dollar_var(ob_term(obs_view(arena.nodes@, out@)[j])) by {
                if j < before_out.len() {
                    obs_wf_at(before_models, j);
                } else {
                    obs_wf_at(right_models, j - before_out.len());
                }
            }
        }
        i += 1;
...
pub fn payload_stage_exec(arena: &mut ETermArena, rows: &Vec<ERow>, pys: &Vec<ESrc>) -> (out:
    Result<Vec<EOb>, EOut>)
    requires
        arena_ok(old(arena)),
        rows.len() == pys.len(),
    ensures
        arena_ok(final(arena)),
        old(arena).nodes@.is_prefix_of(final(arena).nodes@),
        out matches Ok(obs) ==> obs_valid(final(arena).nodes@, obs@) && wf_obs(
            obs_view(final(arena).nodes@, obs@),
        ),
