        ...
        ),
{
    ...
    proof {
        ...
        prefix(n0, arena.nodes@, slot);
        prefix(n0, arena.nodes@, deps);
        prefix(n1, arena.nodes@, &role);
        prefix(n2, arena.nodes@, &doc);
    }
    ...
    args.push(deps.cp());
    proof {
        assert_seqs_equal!(models(args@) == expected);
    }
    let ghost middle = arena.nodes@;
    let out = c(arena, &Sym::DollarGuidelineId, &args);
    proof {
        crate::k2_load::prefix_chain(n0, middle, arena.nodes@);
    }
    out
}
...
pub fn empty(arena: &ETermArena, n: usize) -> (out: Flat)
    requires
        arena_ok(arena),
    ensures
        flat_valid(arena.nodes@, &out),
        out@ == (spec::Flat { items: Seq::empty(), n: n as nat }),
{
    proof {
    ...
    let items = Vec::new();
    proof {
        assert_seqs_equal!(item_models(items@) == Seq::<spec::Item>::empty());
    }
    Flat { items, n }
}
...
pub fn one(arena: &ETermArena, it: I, n: usize) -> (out: Flat)
    ...
    ensures
        flat_valid(arena.nodes@, &out),
        out@ == (spec::Flat { items: seq![it@], n: n as nat }),
{
    let mut items = Vec::new();
    items.push(it);
    proof {
        reveal_with_fuel(items_valid, 2);
        assert(item_models(items@) == seq![it@]);
    }
    Flat { items, n }
}
...
pub fn join(arena: &ETermArena, mut first: Flat, mut last: Flat) -> (out: Flat)
    requires
        arena_ok(arena),
        flat_valid(arena.nodes@, &first),
        flat_valid(arena.nodes@, &last),
    ensures
        flat_valid(arena.nodes@, &out),
        out@ == (spec::Flat { items: first@.items + last@.items, n: last@.n }),
{
    proof {
        items_concat(arena.nodes@, first.items@, last.items@);
    }
...
pub fn flatten_cond(
    ...
    ensures
        ...
        flat_result(out) == spec::flatten_cond(
            c@,
            w@,
            env.s as nat,
            env.docid@,
        ...
        Some((name, a)) => {
            ...
            if has_name(&name, &Sym::Cons) && a.len() == 2 {
                ...
                if let (W::Antecedent, E::Top) = (w, encl) {
                            ...
                            match r {
                                ...
                                Ok(f) => {
                                    if has_anch(arena, &f.items) {
                                        proof {
                                            prefix_all(start, arena.nodes@, b.dom@);
                                        }
                    ...
                    Err(named(arena, &Sym::NafPlacement))
                }
            } else if op(&name) && a.len() == 1 {
                if let E::Naf = encl {
                    return Err(named(arena, &Sym::DeferredOperator));
                }
                match box_parts(arena, &a[0]) {
                    None => Err(named(arena, &Sym::InvalidDrsShape)),
                    Some(b) => {
                        ...
                        proof {
                            ...
                            prefix(start, n2, deps);
                            prefix(start, n2, &b.conds);
                            prefix(n1, n2, &inner);
                        }
                        ...
                        match r {
                            ...
                            Ok(f) => {
                                if has_anch(arena, &f.items) {
                                    proof {
                                        prefix(start, arena.nodes@, outer);
                                        prefix(n1, arena.nodes@, &inner);
                                    }
    ...
    match parts(arena, l) {
        Some((name, a)) => {
            ...
            if has_name(&name, &Sym::Cons) && a.len() == 2 {
                match flatten_cond(arena, &a[0], w, env, deps, outer, encl, n) {
                    ...
                    Ok(f1) => {
                        ...
                        match r {
                            ...
                            Ok(f2) => {
                                proof {
                                    items_prefix(middle, arena.nodes@, f1.items@);
                                }
                                Ok(join(arena, f1, f2))
                            },
                        }
                ...
                Err(named(arena, &Sym::InvalidDrsShape))
            }
        },
        None => Err(named(arena, &Sym::InvalidDrsShape)),
    }
