    ...
    decreases ts.len(),
{
    if ts.len() == 0 {
        ...
        conj_leaves(ts[0]) + leaves_all(ts.drop_first())
    }
}

proof fn leaves_concat(a: Seq<Term>, b: Seq<Term>)
    ensures
        leaves_all(a + b) == leaves_all(a) + leaves_all(b),
    decreases a.len(),
{
    if a.len() > 0 {
...
fn flatten(arena: &ETermArena, root: usize) -> (out: Vec<usize>)
    ...
    ensures
        ...
        root_terms(arena.nodes@, out@) == conj_leaves(arena@[root as int]),
{
    ...
    while tasks.len() > 0
        invariant
            ...
            roots_valid(arena.nodes@, out@),
            conj_leaves(model) == root_terms(arena.nodes@, out@) + leaves_all(
...
fn bind_exec(mut arena: ETermArena, root: usize, vars: &Vec<usize>, values: &Vec<usize>) -> (out: (
    ETermArena,
    usize,
))
    requires
        ...
        roots_valid(arena.nodes@, values@),
    ensures
        arena_ok(&out.0),
        arena.nodes@.is_prefix_of(out.0.nodes@),
        root_ok(&out.0, out.1),
        out.0@[out.1 as int] == bind(
            arena@[root as int],
            root_terms(arena.nodes@, vars@),
            root_terms(arena.nodes@, values@),
            0,
        ),
{
    ...
    while i < vars.len() && i < values.len()
        invariant
            ...
            roots_valid(arena.nodes@, vars@),
            roots_valid(arena.nodes@, values@),
            v == root_terms(origin, vars@),
            v == root_terms(arena.nodes@, vars@),
            vs == root_terms(origin, values@),
            vs == root_terms(arena.nodes@, values@),
            target == bind(origin[root as int].term@, v, vs, 0),
            i <= vars.len(),
            i <= values.len(),
            target == bind(arena@[current as int], v, vs, i as nat),
...
pub enum ERow {
    Row { payload: usize, work: usize, nodes: usize, base: usize },
    Err(EOut),
}

pub open spec fn row_view(nodes: Seq<ENode>, row: &ERow) -> RowOut {
    match row {
        ERow::Row { payload, work, nodes: count, base } => RowOut::Row(
            nodes[*payload as int].term@,
            *work as nat + *count as nat,
            *base as nat,
        ),
...
pub open spec fn row_valid(nodes: Seq<ENode>, row: &ERow) -> bool {
    match row {
        ERow::Row { payload, base, .. } => *payload < nodes.len() && *base <= nodes.len(),
        ERow::Err(_) => true,
    }
}
...
fn unproved_root(arena: &mut ETermArena, finite: bool) -> (out: usize)
    ...
    ensures
        ...
        final(arena)@[out as int] == unproved(
            if finite {
                "finite_failure"@
            } else {
                "limit"@
        ...
        ),
{
    ...
    let why: &[u8] = if finite {
        b"finite_failure"
    } else {
        b"limit"
    };
    proof {
        reveal_byteslit(b"unproved");
        reveal_strlit("unproved");
        ...
        assert(why@ == ckc_spec::v1text::ascii(
            if finite {
                "finite_failure"@
            } else {
                "limit"@
        ...
        ),
{
    ...
    proof {
        ...
        assert((crate::k3_state::out_view(arena.nodes@, &result), left as nat) == trun(
            program,
            roots_cfg(conj_leaves(model)),
            ckc_spec::engine::trace_inf(),
        ));
    }
    match result {
...
pub open spec fn rows_view(nodes: Seq<ENode>, out: &ERows) -> Rows {
    match out {
        ERows::Ok(rs) => Rows::Ok(root_terms(nodes, rs@)),
        ERows::Trip => Rows::Trip,
        ERows::Err(o) => Rows::Err(o@),
    }
}
        ...
        ),
{
    ...
    while i < sols.len()
        invariant
            ...
            root_ok(&arena, goal),
            goal < origin.len(),
            model == arena@[goal as int],
            model == origin[goal as int].term@,
            roots_valid(arena.nodes@, vars@),
            v == root_terms(arena.nodes@, vars@),
            v == root_terms(origin, vars@),
            roots_valid(arena.nodes@, sols@),
            ss == root_terms(arena.nodes@, sols@),
            ss == root_terms(origin, sols@),
            roots_valid(arena.nodes@, acc@),
            i <= sols.len(),
            left <= 1000000,
            base <= arena.nodes.len(),
            target == trace_rows(
        ...
        decreases sols.len() - i,
    {
        ...
        proof {
            assert(before[sols@[i as int] as int].term@ == ss[i as int]);
        }
        ...
        match payload {
            ...
            ERow::Row { payload, work, nodes, base: next } => {
                if work > left || nodes > left - work {
                    return (arena, ERows::Trip);
                }
                left -= work;
                left -= nodes;
                base = next;
...
fn result_inner(
    ...
    query: &EQuery,
    answer: &EAnswers,
) -> (out: (ETermArena, Result<usize, EOut>))
    requires
        arena_ok(&arena),
        db_valid(arena.nodes@, db@),
