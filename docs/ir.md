...
- A rule is exactly `=>(drs(ADom,AConds),drs(CDom,CConds))`. Its consequent denotes exactly one positive head. A one-condition consequent admits `predicate/3`, transitive `predicate/4`, or exact `relation(R1,of,R2)`. A two-condition copular consequent admits exactly one carrier followed by one matching `be`: `object(C,Class,countable,na,eq,1)` plus `predicate(E,be,Subject,C)` becomes unary `pred(Class,[Subject'])`; `property(C,Adj,pos)` plus that `be` becomes unary `pred(Adj,[Subject'])`; comparative `property/4` plus that `be` becomes its binary degree-named head. The carrier and event are consequent-local and erased; semantic arguments must be named or antecedent-bound. A three-condition object+property+`be` attributive consequent would denote two heads and rejects under the existing `consequent_count(3)` single-head law. Head arguments are scanned left-to-right for dense `var(N)` assignment.
...
Lowering is first-failure deterministic. Its outer order is: M2 envelope; root-domain shape; root question count/position; cross-DRS declaration uniqueness; root factual and rule conditions in DRS order; root referent accounting; final-question semantics; output section/order checks; generated-IR validation. An implication owns its root position before anchored-condition dispatch. For an anchored root condition, dispatch is exactly object copula candidate, `be` copula candidate, property copula candidate, transitive `predicate/4`, intransitive `predicate/3`, `relation/3`, then generic unsupported content. The object handler therefore retains unpaired-object-first precedence over any later property or relation. At a property position, adjective separator and degree/arity checks precede carrier binding; a valid carrier then requires exactly one naming `be`, with zero/multiple matches classified `referent`. A transitive predicate proves its event has exactly one condition-tree occurrence before erasing it. Relation separator/name and root argument checks occur at that relation's position.

Within a rule, consequent shape establishes head numbering before antecedent lowering. A recognized two-condition object/property carrier claims copular-head normalization; a malformed pair rejects there. Otherwise exactly one consequent condition is required, so an attributive three-condition consequent fails `consequent_count(3)` before any inner property is emitted. Head arguments and then body arguments scan left-to-right. The positive-before-NAF suffix check precedes body lowering. Antecedent conditions then retain DRS order; a property may consume a matching later `be`, whose event and carrier enter referent accounting while only the semantic property literal enters the body. A recognized outer `~/1` still runs only the two shipped unary profiles, so property content fails as `negation`, not generic `unsupported`. Cross-DRS declaration uniqueness therefore continues to own a `~/1` sub-DRS that redeclares an already-declared referent as `referent` before profile checks.

A final question containing `query/2` enters the exact wh profile before yes/no dispatch; property-based wh therefore remains `wh_query`. Otherwise an exact two-condition object/property carrier is normalized before the single-condition yes/no profiles. Consumed anchors follow the existing one-sentence, deduplicate, and numeric-sort law. If otherwise valid lowering produces a cyclic rule dependency, generated-IR validation returns `lower,cycle`, exit 1; any other generated-record rejection remains an internal invariant failure.
