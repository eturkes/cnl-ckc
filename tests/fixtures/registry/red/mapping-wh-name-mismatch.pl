...
mapping_claim('synthetic-guideline','region-a','claim-a',projection(applicability),docid('doc-a'),items([query_id(sentence(1),clause(2)),rule_id(sentence(1),clause(1))]),expected_answer(answer(query_id(sentence(1),clause(2)),wh(who),pred(eligible,[var(1)]),answers([pred(other,[named(a)])])))).
