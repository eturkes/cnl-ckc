...
mapping_claim('synthetic-guideline','region-a','claim-a',projection(applicability),docid('doc-a'),items([rule_id(sentence(1),clause(1))]),expected_answer(answer(query_id(sentence(1),clause(2)),pred(applies,[named(patient)]),proved))).
