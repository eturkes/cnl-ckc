...
closed_world(exception_id(rule(rule_id(sentence(3),clause(1))),literal(1)),affects(rule_id(sentence(3),clause(1))),predicate_key(smoker,arity(1))).
...
rule(rule_id(sentence(3),clause(1)),pred(recover,[var(1)]),body([pred(patient,[var(1)]),naf(exception_id(rule(rule_id(sentence(3),clause(1))),literal(1)),pred(smoker,[var(1)]))]),source(sentence(3),tokens([2,4,8,9]))).
