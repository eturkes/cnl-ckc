...
fact(fact_id(sentence(1),clause(2)),pred('high comp',[named('Dose')]),source(sentence(1),tokens([2,4]))).
rule(rule_id(sentence(2),clause(1)),pred(preferred,[var(1)]),body([pred(dosage,[var(1)]),pred('high comp',[var(1)])]),source(sentence(2),tokens([2,3,4,5]))).
