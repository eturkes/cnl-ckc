...
fact(fact_id(sentence(2),clause(1)),temporal_window(before,pred(wait,[named('John')]),anchor(named('Therapy-start')),interval(quantity_bound(geq,closed,quantity(integer(1),unit(week))),quantity_bound(leq,closed,quantity(integer(4),unit(week))))),source(sentence(2),tokens([1,2,3,5,6,7,8]))).
rule(rule_id(sentence(3),clause(1)),pred(recover,[var(1)]),body([pred(patient,[var(1)]),temporal_window(before,pred(wait,[var(1)]),anchor(named('Therapy-start')),interval(quantity_bound(geq,closed,quantity(integer(1),unit(week))),quantity_bound(leq,closed,quantity(integer(4),unit(week)))))]),source(sentence(3),tokens([2,4,5,6,7,8,9,10,12]))).
