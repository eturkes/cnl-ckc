...
def _statement(s: _Stream):
    kind = s.peek().kind
    # v0.2.0 code slot — a `{{ ... }}` at statement position (first token on
    # its own line, no preceding verb) resolves to one or more Python
    # statements at the surrounding block's indentation. Expression slots
    # remain reachable via `_operand` (right-hand side of Set, keyword-arg
    # value, list item, etc.). Position discriminator lives on the AST node
    # so the emitter picks the right resolver mode.
    if kind == "SLOT":
        tok = s.next()
        s.expect("NEWLINE")
...
def _statement_head(s: _Stream):
    ...
    """
    kind = s.peek().kind
    # v0.2.0 — a bare `{{ ... }}` on its own line is a canonical code-slot
    # statement head.
    if kind == "SLOT":
        s.next()
        return
