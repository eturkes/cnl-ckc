...
def _emit_stmt(stmt, level, resolve_slot):
    ...
    if isinstance(stmt, Define):
        ...
        return [head] + _emit_body(stmt.body, level + 1, resolve_slot)
    if isinstance(stmt, LlmSlot):
        # v0.2.0 code slot — position="statement" reaches the emitter here
        # (statement-list branch); position="expression" is handled inside
        # _emit_expr. Resolver returns raw Python source; splice at the
        # surrounding block's indentation and preserve blank lines the
        # author (or LLM) intended.
        try:
            resolved = resolve_slot(stmt.text, position="statement")
        except TypeError:
            # Back-compat with resolvers that predate the position param.
            resolved = resolve_slot(stmt.text)
        out = []
        for raw in str(resolved).splitlines():
            # rstrip only — leave any interior leading whitespace on
            # continuation-lines alone; indent uniformly with pad.
            if raw.strip() == "":
                out.append("")  # preserve blank line, no indentation
            else:
                out.append(f"{pad}{raw.rstrip()}")
        return out
...
def _emit_expr(node, resolve_slot):
    ...
    if isinstance(node, LlmSlot):
        # v0.2.0 — pass `position="expression"` when the resolver signature
        # supports it. Existing resolvers that only accept `text` continue
        # to work via the TypeError fallback.
        try:
            resolved = resolve_slot(node.text, position="expression")
        except TypeError:
