...
- {{ english phrase }} is an LLM slot — keep such phrases verbatim inside {{ }} (do not resolve them). Two positions:
    * VALUE position (inside a Set/keyword-arg/list): {{ ... }} resolves to a Python expression.
    * STATEMENT position (a whole line, at block indentation, with nothing before or after): {{ ... }} resolves to one or more Python statements. Emit statement-position slots as their own canonical line.
