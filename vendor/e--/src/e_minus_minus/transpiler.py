...
def _default_resolver(text: str, position: str = "expression") -> str:
...
def transpile(source: str, resolve_slot=None) -> str:
    ...
    ``resolve_slot`` is a callable invoked for each ``{{ ... }}`` slot with
    signature ``resolve_slot(text: str, position: str = "expression") -> str``.
    - Expression position (v0.1.0): return value is a Python expression.
    - Statement position (v0.2.0): return value is one or more Python
      statements. Emitter splices them at the surrounding block's
      indentation.

    Callers on the v0.1.0 signature (``resolve_slot(text)``) continue to work:
    the emitter tries the v0.2.0 kwarg first and falls back on ``TypeError``.
    """
