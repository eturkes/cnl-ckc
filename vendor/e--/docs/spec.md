...
### Slot positions (v0.2.0)

A `{{ ... }}` slot may appear at two positions, and its position determines
the semantics of resolution:

- **Expression position** — inside an expression (right-hand side of `Set`,
  a keyword-argument value, a list item, a return value). Resolves to a
  single Python expression via `resolve_slot(text, position="expression")`.
- **Statement position** — as its own line at the current block's indentation
  (nothing before or after on that line except whitespace). Resolves to one
  or more Python statements via `resolve_slot(text, position="statement")`.

The resolver contract is:

```
resolve_slot(text: str, position: str = "expression") -> str
```

The default value preserves back-compat for v0.1.0 callers with a
single-argument signature: the emitter tries the kwarg and falls back to
`resolve_slot(text)` on `TypeError`.

**Graph visibility trade-off**. Any `[[wikilinks]]` or callable references
inside a statement slot's resolved Python are **opaque** to any downstream
tool that reads the E-- source only. The transpiler emits Python; it doesn't
build a dependency graph. Use expression slots when graph visibility matters;
use statement slots when you're delegating region-level structure the LLM
