...
## Pre-prompt-drafting staleness check (HARD RULE)

Before drafting ANY new prompt — bug fix, feature, polish, doc, anything — cowork confirms it is not working against a stale spec or protocol. The discipline is mandatory; the *cost* is sized to what actually changed.

**The check (cheap path, default).** At the start of drafting a prompt, for both `~/projects/e--/docs/spec.md` and `~/projects/e--/docs/cowork-protocol.md`:

1. Get the file's current HEAD SHA: `git log -1 --pretty=%H -- docs/spec.md` (and the same for the protocol file).
2. Compare each against the **SHA recorded when cowork last read that file** — store that SHA; do not rely on memory (same "read git state, never recall it" discipline CC follows).
3. SHA unchanged → that file is current in head; no re-read needed.
4. SHA changed → read what changed. For the spec, the **Changelog delta** is the authoritative summary of what landed since the last-read SHA (`git diff <last-read-SHA>..HEAD -- docs/spec.md`, or read the new changelog lines). For the protocol, read the changed sections.

**When a full end-to-end re-read is required** (not just the delta):

- the changelog delta is non-trivial — a `breaking:` entry, a grammar restructure, a new section; OR
- cowork has not read the file at all this session; OR
- the diff touches a section the pending prompt depends on.

The changelog is trusted here for the same reason it is the downstream **stability contract** (see "Downstream consumers"): if it is reliable enough for consumers to pin releases against, it is reliable enough to tell cowork what changed. A full re-read is the belt-and-suspenders case, not the default.

**Why this is a hard rule.** The failure mode — drafting against a stale spec or protocol — produces wasted drains, mid-flight aborts to `questions/`, and cowork eating its own work. The cheap check costs two `git log` calls when nothing changed; it is never skippable, only sized to the delta.

**Cross-project note**: this discipline mirrors the sibling Forge project's same mandate. The two projects' coworks operate independently; the discipline travels because the failure mode is universal, not because Forge imposes anything on E-- (and per Downstream consumers, E-- cowork authors E--'s own protocol edits).
...
**Coupling is one-directional.** Downstream consumers depend on E--; E-- never depends on them. E-- never imports from a consumer's codebase, never reads consumer-side test results, never accepts consumer-imposed design constraints. The implementation, spec, mission, release cadence, and protocol files in `e--/` are owned by E-- cowork alone. Consumers may *suggest* protocol patterns — shared failure modes travel — but **E-- cowork authors the E-- edit**; a consumer does not write into E--'s governance files directly.
