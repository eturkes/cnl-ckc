...
### Re-read this file at the start of every drain invocation (HARD RULE; first action before anything else)

Even within a long-running interactive CC session, re-read `~/projects/e--/docs/cc-prompt-queue.md` fresh when the user says "do prompt", "drain prompts", or any equivalent. The file may be amended between drains based on what prior drains surfaced — new hard rules land here as the project matures. Stale-protocol-context is a real failure mode and several Forge drain mistakes (the sibling project) traced to it; same pattern applies to any project using this protocol shape.

Make the re-read the **first action** of every drain, before reading the prompt itself, before reading any other context. If you find a rule you didn't know existed, that's the rule firing correctly.
