...
from e_minus_minus.errors import EmmSyntaxError  # noqa: E402
...
_CLI_MODULE = "e_minus_minus.transpiler"
# Env for CLI subprocess: put the package's parent (`src/`) on PYTHONPATH so
# `python -m e_minus_minus.transpiler` finds the package without an install.
_CLI_ENV = dict(os.environ, PYTHONPATH=os.path.join(_REPO_ROOT, "src"))
...
class TestCliRunner(unittest.TestCase):
    ...
    def test_cli_transpile_stdout(self):
        try:
            proc = subprocess.run(
                [sys.executable, "-m", _CLI_MODULE, _DESCRIBE_EMM],
                capture_output=True, text=True, cwd=_REPO_ROOT, env=_CLI_ENV)
    ...
    def test_cli_run_executes(self):
        try:
            proc = subprocess.run(
                [sys.executable, "-m", _CLI_MODULE, _DESCRIBE_EMM, "--run"],
                capture_output=True, text=True, cwd=_REPO_ROOT, env=_CLI_ENV)
        except OSError as exc:
            self.skipTest(f"cannot spawn subprocess: {exc}")
        self.assertEqual(proc.returncode, 0, proc.stderr)
    ...
    def test_cli_missing_file(self):
        try:
            proc = subprocess.run(
                [sys.executable, "-m", _CLI_MODULE, "does/not/exist.emm"],
                capture_output=True, text=True, cwd=_REPO_ROOT, env=_CLI_ENV)
        except OSError as exc:
            self.skipTest(f"cannot spawn subprocess: {exc}")
    ...
    def test_cli_run_show_prints_code_then_output(self):
        try:
            proc = subprocess.run(
                [sys.executable, "-m", _CLI_MODULE, _DESCRIBE_EMM, "--run", "--show"],
                capture_output=True, text=True, cwd=_REPO_ROOT, env=_CLI_ENV)
        except OSError as exc:
            self.skipTest(f"cannot spawn subprocess: {exc}")
        self.assertEqual(proc.returncode, 0, proc.stderr)
    ...
    def test_cli_show_alone_does_not_run(self):
        try:
            proc = subprocess.run(
                [sys.executable, "-m", _CLI_MODULE, _DESCRIBE_EMM, "--show"],
                capture_output=True, text=True, cwd=_REPO_ROOT, env=_CLI_ENV)
        except OSError as exc:
            self.skipTest(f"cannot spawn subprocess: {exc}")
        self.assertEqual(proc.returncode, 0, proc.stderr)
...
def _fake_positioned(text, position="expression"):
    """Fake resolver that returns literal Python per (text, position) pair.

    Kept small: only handles the phrases the code-slot tests use.
    """
    expr_mapping = {
        "the first prime number greater than 5": "7",
        "a calm blue": '"blue"',
    ...
    stmt_mapping = {
        ...
        "assign counter to zero": "counter = 0",
        # Multi-statement, one binding.
        "print two friendly greetings":
            'print("hello")\nprint("world")',
        # Binds a name that later canonical code references.
        "assign total to the sum of one two and three":
            "total = 1 + 2 + 3",
        # For inside a loop.
        "print the current index in the loop":
            "print(i)",
        # Inside an If.
        "log positive": 'print("positive")',
    }
    if position == "expression":
        if text in expr_mapping:
            return expr_mapping[text]
    else:
        if text in stmt_mapping:
            return stmt_mapping[text]
    raise AssertionError(
        f"fake_positioned has no {position} mapping for {text!r}")


def _t2(src):
    return transpile(src, resolve_slot=_fake_positioned)


class TestCodeSlotStatements(unittest.TestCase):
    def test_statement_slot_top_level_single_line(self):
        src = "{{ assign counter to zero }}\n"
        self.assertEqual(_t2(src), "counter = 0")

    def test_statement_slot_top_level_multi_line(self):
        src = "{{ print two friendly greetings }}\n"
        expected = 'print("hello")\nprint("world")'
        self.assertEqual(_t2(src), expected)

    def test_statement_slot_binds_name_used_by_later_canonical(self):
        src = (
            "{{ assign total to the sum of one two and three }}\n"
            "Do [[print]](total).\n"
        )
    ...
    def test_statement_slot_inside_foreach_indented(self):
        src = (
            "For each i in items:\n"
            "    {{ print the current index in the loop }}\n"
        )
    ...
    def test_statement_slot_inside_if_otherwise(self):
        src = (
            "If x is greater than 0:\n"
            "    {{ log positive }}\n"
            "Otherwise:\n"
    ...
    def test_expression_slot_still_works_regression(self):
        # Regression: v0.1.0 expression-slot behavior unchanged.
        src = "Set n to {{ the first prime number greater than 5 }}."
        self.assertEqual(_t2(src), "n = 7")


class TestCodeSlotBackCompat(unittest.TestCase):
    def test_v01_resolver_without_position_still_works(self):
        # v0.1.0 resolvers (single-arg signature) continue to work — the
        # emitter falls back on TypeError.
        def v01_only(text):
            mapping = {"the first prime number greater than 5": "7"}
            return mapping[text]

        result = transpile(
            "Set n to {{ the first prime number greater than 5 }}.",
            resolve_slot=v01_only,
        )
