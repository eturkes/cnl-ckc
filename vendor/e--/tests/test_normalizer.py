...
from e_minus_minus.normalizer import make_normalizer  # noqa: E402
from e_minus_minus.transpiler import transpile  # noqa: E402
from e_minus_minus.parser import is_canonical_statement_line  # noqa: E402
from e_minus_minus.errors import EmmNormalizeError  # noqa: E402
...
class TestNormalizer(unittest.TestCase):
    ...
    def test_cli_dual_output_canonical_file(self):
        ...
        try:
            proc = subprocess.run(
                [sys.executable, "-m", "e_minus_minus.transpiler",
...
class TestStatementSlotCanonicalDetection(unittest.TestCase):
    """v0.2.0 — a bare ``{{ ... }}`` on its own line is a canonical
    statement-position code slot. The normalizer relies on this
    classification to leave code-slot lines verbatim rather than treating
    them as free English to re-normalize."""

    def test_statement_slot_line_is_canonical(self):
        self.assertTrue(
            is_canonical_statement_line("{{ assign counter to zero }}"))
        self.assertTrue(
            is_canonical_statement_line(
                "    {{ do something with items }}"))
        self.assertTrue(
            is_canonical_statement_line("{{ multi word phrase here }}"))

    def test_non_slot_lines_still_classify(self):
        # Regression: adding statement-slot support hasn't reclassified any
        # non-slot lines.
        self.assertTrue(is_canonical_statement_line("Set x to 1."))
        self.assertFalse(is_canonical_statement_line("just prose"))
